...
//			fileKey = "clt05_001";
			...
			forward="/order/3DcustomSelect.jsp";
		}
		// }
...
//			forward = "inputCustomOrder";
			...
			return new ActionForward("/CustomOrderAction.do?method=addClient&tp=3Dcus&type=firm");
		}
		...
		try {
			...
			if (returnValue.isSucessFlag()) {
				...
				String tmkfno2 = (String) ((HashMap) resEnv.getBody())
						.get("tmkfno2");
				if(tp.equals("3d")){
					List<String> fnoList = new ArrayList<String>();
					if(!tmkfno2.equals("")){
						fnoList.add(tmkfno2);
					}
					if(!tmkfno1.equals("")){
					...
					String[] tmkfno = (String[])fnoList.toArray(new String[0]);
					// 文件上传
					// 获取文件数目
					int size = orderForm.getFileCount();
					
					for (int i = size-1; i > -1; i--) {
						//获取文件名
						String name = orderForm.getFile(i).getFileName();
						String suffix = "";
						if(name.contains(".")){
						...
						//获取文件二进制流
						byte[] b = orderForm.getFile(i).getFileData();
						//创建目录,目录格式为“3Dfiles/客户id/订单id”
						File dest = new File(GlobalNames.SERVER_1_PAN + ":\\" + GlobalNames.SERVER_1_DIR +"\\" + cid + "\\" + tmkfno[i]);
						if(!dest.exists()){
							dest.mkdirs();
						}
						//写入文件
						File file = new File(GlobalNames.SERVER_1_PAN + ":\\" + GlobalNames.SERVER_1_DIR +"\\" + cid + "\\" + tmkfno[i] + "\\" + order.getIctnm() + "_" + tmkfno[i] + "_" + order.getFoldt() + suffix);
						if (!file.exists())
							file.createNewFile();
						FileOutputStream out = new FileOutputStream(file, true);
						out.write(b);
						/*Connection conn = DBUtil.getConnection();
						DBUtil.execSQL(conn , "update tblfolio set folfile =  '" + name + "' where folno = '" + tmkfno[i] + "'");
						conn.close();*/
					}
				}
				...
				if(tp.equals("3d")){
					return mapping.findForward("3DcustomSelect");
				}else{
...
//		SubmitDataMap data=new SubmitDataMap(req);
		...
		Order order = new Order();
		Diagnose dg = new Diagnose();
		System.out.println(GlobalNames.SERVER_1_PAN);
		...
		try {
			...
			if (returnValue.isSucessFlag()) {
				...
				for (int i = size-1; i > -1; i--) {
					...
					byte[] b = orderForm.getFile(i).getFileData();
					//创建目录,目录格式为“3Dfiles/客户id/订单id”
					File dest = new File(GlobalNames.SERVER_1_PAN + ":\\"+GlobalNames.SERVER_1_DIR+"\\" + order.getIctid() + "\\" + order.getFolno());
					if(!dest.exists()){
						dest.mkdirs();
					}
					File[] tempList = dest.listFiles();
					for(int j = 0 ; j < tempList.length ; j ++){
						File temp = tempList[j];
						temp.delete();
					}
					//写入文件
					File file = new File(GlobalNames.SERVER_1_PAN + ":\\"+GlobalNames.SERVER_1_DIR+"\\" + order.getIctid() + "\\" + order.getFolno() + "\\" + name);
					if (!file.exists())
						file.createNewFile();
					FileOutputStream out = new FileOutputStream(file, true);
					out.write(b);

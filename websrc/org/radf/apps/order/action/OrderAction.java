...
public class OrderAction extends ActionLeafSupport {
			...
			if ("nomRecExa".equals(order) || "cusRecExa".equals(order)) {
				...
				if ((null != orderForm.getFoldtty() && !"".equals(orderForm
						...
						&& (null != orderForm.getEnd() && !"".equals(orderForm
								.getEnd()))) {
					hql += " and " + orderForm.getFoldtty() + "<=to_date('"
							+ orderForm.getEnd() + " 23:59:59','yyyy-MM-DD HH24:mi:ss')";
				}
				
				if("chgdt".equals(orderForm.getNcddtty())){
					
					if ((null != orderForm.getNcddtty() && !"".equals(orderForm
							.getNcddtty()))
							&& (null != orderForm.getStart() && !""
									.equals(orderForm.getStart()))) {
						hql += " and r." + orderForm.getNcddtty() + ">=to_date('"
								+ orderForm.getStart() + "','yyyy-MM-DD')";
					}
					if ((null != orderForm.getNcddtty() && !"".equals(orderForm
							.getNcddtty()))
					...
					if ((null != orderForm.getNcddtty() && !"".equals(orderForm
							...
							&& (null != orderForm.getStart() && !""
									.equals(orderForm.getStart()))) {
						hql += " and l." + orderForm.getNcddtty() + ">=to_date('"
								+ orderForm.getStart() + "','yyyy-MM-DD')";
					}
					if ((null != orderForm.getNcddtty() && !"".equals(orderForm
							.getNcddtty()))
				...
				if(tp!=null){
					if (tp.equals("e")){
						if (null != orderForm.getStart() && !"".equals(orderForm.getStart())) {
							hql += " and h.foldt>=to_date('"
									+ orderForm.getStart() + "','yyyy-MM-DD')";
						}
						if (null != orderForm.getEnd() && !"".equals(orderForm.getEnd())) {
							hql += " and h.foldt<=to_date('"
									+ orderForm.getEnd() + " 23:59:59','yyyy-MM-DD HH24:mi:ss')";
						}
						hql+= "order by h.foldt";
					}
				}
			}
	...
	public ActionForward orderDetail(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (order.getFoltype().equals("make")) {
				if("s".equals(tp2)&&orderForm.getTdefolno()!=null&&orderForm.getTdefolno().length()>0){
					order.setFolno(orderForm.getTdefolno());
					order.setFolsta("");
				}
				resEnv = facade.printCustomDetail(requestEnvelop);
			}else if (order.getFoltype().equals("3DMake")) {
				resEnv = facade.printCustomDetail(requestEnvelop);
			...
			if (returnValue.isSucessFlag()) {
				...
				if (order.getFoltype().equals("make")||order.getFoltype().equals("3DMake")) {
					for (Map.Entry<String, Object> entry : mapResponse
							.entrySet()) {
				...
				if (order.getFoltype().equals("make")
						|| order.getFoltype().equals("3DMake")
				...
				//审核订单？
					...
					if (order.getFoltype().equals("make")) {
						forward = "customDetail";
					} if (order.getFoltype().equals("3DMake")) {
						forward = "3DcustomDetail";
					}else if (order.getFoltype().equals("repair")) {
						// forward="cusRepDetail";
	...
	public ActionForward printClient(ActionMapping mapping, ActionForm form,
		...
		}else if (tp.equals("3DCus")){
			forward = "confirmClient3DCus";
		}
	...
	public ActionForward updateOrder(ActionMapping mapping, ActionForm form,
		...
		String fdtfno = req.getParameter("folno");//订单编号
		String cltnm = req.getParameter("cltnm");//个人客户
		String fdtpid = req.getParameter("fdtpid");//商品代码
		String fdtdprc = req.getParameter("fdtdprc");//价格
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = DBUtil.getConnection();
			DBUtil.beginTrans(con);
			String update_sql1 = "UPDATE TBLFOLDETAIL SET FDTCLTNM = '"+cltnm+"',FDTPID = '"+fdtpid+"',FDTPRC = '"+fdtdprc+"' WHERE FDTFNO = '"+fdtfno+"' ";
			pstm = con.prepareStatement(update_sql1);
			pstm.executeUpdate();
			DBUtil.commit(con);	
		} catch (Exception e) {
			super.saveErrors(req, e);
	...
	public ActionForward CopyOrder(ActionMapping mapping, ActionForm form,
		...
		String[] fdtfno = data.getParameterValues("folno"); 
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			con = DBUtil.getConnection();
			DBUtil.beginTrans(con);
			for(int i =0;i < fdtfno.length;i++)
			{
				String sql1 = "insert into TBLFOLIOCOPY select * from TBLFOLIO where FOLNO = '"+fdtfno[i]+"' ";
		...
		super.saveSuccessfulMsg(req, "备份成功");
		return mapping.findForward("backspace");
	}
	/*  public ActionForward editDetail(ActionMapping mapping, ActionForm form,
	  HttpServletRequest req, HttpServletResponse res) throws Exception {
		  ...
		  String oprnm = req.getParameter("bsc012");
		  OrderHeaderForm orderForm = (OrderHeaderForm) form; 
		  OrderDetail od = new OrderDetail(); 
		  ActionForward af = new ActionForward();
		  od.setFdtfno(folno); 
		  orderForm.setFolctnm(gctnm); 
		  String forward = "/order/editDetail.jsp"; // 修改订单明细
		  try {  
		      od.setFileKey("ord01_001");
...
*/
	 ...
	 * 查询订单退机基本信息（退机数据页面）
	 */
	public ActionForward requery2(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		String forward = "/order/redate.jsp";
		String fileKey = null;
		ActionForward af = new ActionForward();
		OrderHeaderForm orderForm = (OrderHeaderForm) form;
		Order or = new Order();
		String beginend = req.getParameter("beginend");
		
		String tmksid = orderForm.getSid();
		String cltnm = orderForm.getCltnm();
		fileKey = "ord05_003"; // 查询要退机的订单
		System.out.println("requery2");
		try {
			ClassHelper.copyProperties(orderForm, or);
			String foldreason = req.getParameter("foldreason");
			String type = req.getParameter("beginend");
			String start = req.getParameter("start");
			String end = req.getParameter("end");
			String storetype = req.getParameter("storetype");
			String mtype = req.getParameter("mtype");
			or.setFdtres(foldreason);
			or.setFileKey(fileKey);
			String hql = queryEnterprise(or);
			
			if(!storetype.equals("") && storetype != null){
				hql += " and s.bsc005 = '"+storetype+"'";
			}
			if(mtype.equals("BTE")){
				hql += " and p.pdtcls ='BTE'";
			}else if(mtype.equals("DZ")){
				hql += " and p.pdtcls ='CIC' OR p.pdtcls ='ITE' OR p.pdtcls ='ITC' OR p.pdtcls ='HS'";
			}
			if(!type.equals("") && type != null){
				if (null != start && !"".equals(start)) {
					hql += " and "+ type + " >=to_date('"+ start + "','yyyy-MM-DD')";
				}
				if (null != end && !"".equals(end)) {
					hql += " and "+ type + " <=to_date('"+ end + " 23:59:59','yyyy-MM-DD HH24:mi:ss')";
				}
			}
			System.out.println(hql);
			af = super.init(req, forward, hql);
	...
	public ActionForward examineOrder(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			} else if ("nom".equals(folType) && tp1.equals("e")) {//订单类型为普通订单
	...
	public ActionForward inDetail(ActionMapping mapping, ActionForm form,
		...
		String type=req.getParameter("type");
		if("RIC".equals(type)){
			forward = "/order/inputNomOrder2.jsp";
		}
		try {
	 ...
	 * 打印3d订单
	 */
	public ActionForward print3DOrderReport(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		OrderHeaderForm orderForm = (OrderHeaderForm) actionForm;
		Order order = new Order();
		...
		String forward = null;
		String fileKey = null;
		if (!(orderForm.getFoltype().equals("3DMake"))) {
			super.saveSuccessfulMsg(req, "该订单不是3D订单，不能打印！");
		...
		try {
			// 报表编译之后生成的.jasper 文件的存放位置
			File reportFile = new File(req.getSession().getServletContext()
					.getRealPath("/WEB-INF/report/order_3DOrder.jasper"));
			String webRootPath = req.getSession().getServletContext()
					.getRealPath("/");
	...
	public ActionForward downloadFile(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		String folno = req.getParameter("folno2");
		Connection conn = DBUtil.getConnection();
		Vector<HashMap> result = new Vector<HashMap>();
		//通过订单id获得客户id
		result = (Vector<HashMap>)DBUtil.querySQL(conn, "select distinct(FDTCLTID) as cid from TBLFOLDETAIL where FDTFNO = '" + folno + "'");
		HashMap map = result.get(0);
		String cid = (String) map.get("1");
		//拼接目录
		String dest = GlobalNames.SERVER_1_PAN + ":\\" + GlobalNames.SERVER_1_DIR +"\\" + cid + "\\" + folno + "\\";
		//得到目录下的文件列表
		File file = new File(dest);
		File[] tempList = file.listFiles();
		 //读取文件  
        InputStream in = new FileInputStream(tempList[0]);  
        OutputStream out = res.getOutputStream();  
      //设置文件MIME类型  
        res.setContentType(req.getSession().getServletContext().getMimeType(tempList[0].getName()));  
        //设置Content-Disposition  
        res.setHeader("Content-Disposition", "attachment;filename=" + new String (tempList[0].getName().getBytes("gb2312"),"ISO8859-1"));  
        //写文件  
        int b;  
        while((b=in.read())!= -1)  
        {  
	...
	public ActionForward queryToModity(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		ActionForward af = new ActionForward();
		OrderHeaderForm orderForm = (OrderHeaderForm) form;
		Order or = new Order();
		...
		String fileKey = "ord02_004";
		String forward = "/order/queryToModify.jsp";
	...
	public ActionForward modifyStatus(ActionMapping mapping, ActionForm form,
			...
			requestEnvelop.setBody(mapRequest);
			// 调用对应的Facade业务处理方法
			ResponseEnvelop resEnv = facade.queryOrder(requestEnvelop);
			// 处理返回结果
			returnValue = processRevt(resEnv);
			
			if (returnValue.isSucessFlag()) {
				HashMap mapResponse = (HashMap) resEnv.getBody();
				//List list = (ArrayList) mapResponse.get("beo");
				List<Order> list = (ArrayList<Order>) ((HashMap) resEnv
						.getBody()).get("beo");
				//ClassHelper.copyProperties(list.get(0), orderForm);
				ClassHelper.copyProperties(list.get(0), form);
				
				ClassHelper.copyProperties(list.get(0), or);
								
				req.getSession().setAttribute("sta",or.getFolischg());
				req.getSession().setAttribute("folstaorigin",or.getFolsta());
				
				//return mapping.findForward("modifyOrderStatus");
				//return new ActionForward("/modifyOrderStatus.jsp");
				or.setFileKey("order_and_store_select");
				String forward = "/order/modifyOrderStatus.jsp";
				String hql = queryEnterprise(or);
				ActionForward af = new ActionForward();	
				af = super.init(req, forward, hql);
				return af;
	...
	public ActionForward savemodifyStatus(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		
		String fdtfno = req.getParameter("fdtfno");//订单编号
		String folsta = req.getParameter("folsta");//修改为的订单状态
		String folstaorigin = req.getParameter("folstaorigin");//初始的订单状态
		String sta = req.getParameter("sta");//是否已收费
		SubmitDataMap data = new SubmitDataMap(req);
		String[] codeList = data.getParameterValues("stoid"); //库存编号
		Connection con = null;
		
		try {

			if(folsta.equals("charged")){//若修改为已收费状态
				if(sta.equals("1")){//判断是否已收费
					
					con = DBUtil.getConnection();
					
					//设定订单状态
					String update_sql1 = "UPDATE TBLFOLIO SET FOLSTA = '"+folsta+"' WHERE folno = '"+fdtfno+"' ";
					DBUtil.execSQL(con, update_sql1);
					
					//清空退机信息
					String update_sql2 = "UPDATE TBLFOLDETAIL SET FDTODT = '',FDTRECMON = '',FDTRECHEADDT = '',FDTEXADT = ''  WHERE fdtfno = '"+fdtfno+"' ";
					DBUtil.execSQL(con, update_sql2);
					...
					if(codeList.length != 0){
						for(int i = 0;i<codeList.length;i++){
							stoid = codeList[i];
							delete_sql="DELETE from TBLSTO where stoid = '"+stoid+"' ";
							DBUtil.execSQL(con, delete_sql);
						}
					}
					...
					super.saveSuccessfulMsg(req, "该订单未收费，不能修改为已收费");
					return mapping.findForward("backspace");
					
				}
			}else if(folsta.equals("finish")){//若修改为已发货状态
				...
				//清空收费信息
				String update_sql3 = "UPDATE TBLFOLIO SET FOLCHGDT = '',FOLISCHG = '0' WHERE folno = '"+fdtfno+"' ";
				DBUtil.execSQL(con, update_sql3);
				...
				super.saveSuccessfulMsg(req, "不支持修改为该订单状态");
				return mapping.findForward("backspace");
	...
	public ActionForward printNorBeforeVerify(ActionMapping mapping, ActionForm form,
		...
		String gctnm = req.getParameter("gctnm");
		String foltype = req.getParameter("foltype");
		String fdtcltnm = req.getParameter("fdtcltnm");
		String start = req.getParameter("start");
		String end = req.getParameter("end");
		...
		try {
			...
			reportFile = new File(req.getSession().getServletContext()
						.getRealPath("\\WEB-INF\\report\\printBeforeVerify.jasper"));
			
			// 传递报表中用到的参数值
			Map<String, Object> parameters = new HashMap<String, Object>();
			// "Name"是报表中定义过的一个参数名称,其类型为String 型
/*			parameters.put("folno", folno);
			parameters.put("gctnm", gctnm);
			parameters.put("foltype", foltype);
			parameters.put("fdtcltnm", fdtcltnm);
			...
			PrintBeforeVerifyFactory pf = new PrintBeforeVerifyFactory();
			List<PrintBeforeVerify> data = pf.setCollection(folno, gctnm, foltype,fdtcltnm,start,end);

...
import org.radf.apps.commons.entity.OrderAndStore;
...
import org.radf.plat.commons.UploadFile;
...
//					store.setStodate(DateUtil.getDate());
	    			...
	    			//直属店入库
	    			if(null != type && "make".equals(type) || "makeEar".equals(type) || "normal".equals(type) ||"repair".equals(type)|| "repairEar".equals(type)){//A代表该团体客户为直属店并且订单类型为定制订单、耳模订单或普通订单，则入其库。
...
//	    				Integer stoAmount = extendedAmount(con, od);
						...
						if(null != type && "make".equals(type) || "makeEar".equals(type) ||"repairEar".equals(type) ||"repair".equals(type)){
							if("make".equals(type)){
								store.setStoremark("定制机入库");
							}
							else if("makeEar".equals(type)){
								store.setStoremark("耳模入库");
							}
							else if("repairEar".equals(type)){
								store.setStoremark("维修耳模入库");
							}
							else{
								store.setStoremark("维修机入库");
								store.setIsrepair("1");
							}
						}else{
...
//			header.setFolctid(gid); // 团体客户
			header.setFoldt(DateUtil.getSystemCurrentTime()); // 订单创建日期
			header.setFoldctime(DateUtil.getSystemTime()); // 订单创建时间
...
//	        or.setFoldctime(DateUtil.getSystemTime());
...
//	             {
//		            sql2+="and e.dgnlid='"+fdtp id+"'"; dgncldprc
//	             } 
//		         else
//		         {
...
//				ord.setFileKey("ord11_001");// 修改主订单状态
				ord.setFolsta("pass");//修改订单状态
				ord.setFolopr(opr);//操作人员
...
//					modify(con,rep,null,0);
				...
				//订单类型为定制订单。
				else if(null != ord.getFoltype() && (ord.getFoltype().equals("make")||ord.getFoltype().equals("3DMake")) )
				{
...
//					if (null != od.getFdtsheltpl() && !"".equals(od.getFdtsheltpl())) {
					if(od.getFdtmklr().equals("0")){//判断左右耳
						proc.setInt(1, 0);//设置存储过程左右耳
						cz.setTmkplr("0");//设置左右耳
						cz.setTmkpid(od.getDgnlid());//耳机代码，产品编号
...
//					modify(con, ord, null, 0);
					proc.registerOutParameter(2, Types.VARCHAR);//获得传出参数
					proc.setString(3, type);//设置存储过程类型参数
					proc.execute();
					retTmksid = proc.getString(2);//得到机身编号
...
//					modify(con,em,null,0);
				...
				modify(con, ord, null, 0);  
				//订单类型为3D定制订单
				if(null != ord.getFoltype() && ord.getFoltype().equals("3DMake")){
					//需要增加条形码生成相关代码
					
					//将数据同步到3D订单平台
					proc = con.prepareCall("{ call INFO_TO_3DMAKE(?,?) }");
					proc.setString(1, ord.getFolno());
					proc.setString(2, ord.getIctid());
					//proc.registerOutParameter(1, Types.VARCHAR);
					proc.execute();
					
					
					/*//拼接目录，源文件的存放位置
					String dest = GlobalNames.SERVER_1_PAN + ":\\"+ GlobalNames.SERVER_1_DIR +"\\" + ord.getIctid() + "\\" + ord.getFolno() + "\\";
					//得到目录下的文件列表
				    ...
				    boolean flag1 = UploadFile.uploadFile(GlobalNames.SERVER_2_IP, Integer.parseInt(GlobalNames.SERVER_2_PORT), GlobalNames.FTP_USER, GlobalNames.FTP_PASSWORD, GlobalNames.SERVER_2_DIR, new String(tempList[0].getName().toString().getBytes("GBK"),"iso-8859-1"), bin,ord.getIctid(),ord.getFolno());  
...
//			header.setFolno(tmkfno1); // 订单号
			...
			header.setFoltype(tp.equals("3d") ? "3DMake" : "make");// 订单类别为"定制机订单"或者“3d定制订单”
			//header.setFolopr(opr);
...
//					}
		...
		@Override
		public ResponseEnvelop queryOrder(RequestEnvelop request) {
			ResponseEnvelop response = new ResponseEnvelop();
			Connection con = null;
			try {
				con = DBUtil.getConnection();
				...
				HashMap map = (HashMap) request.getBody();
				Order dto1 = (Order) map.get("beo");
				
				dto1.setFileKey("order_and_store_select");	
				Object order = find(con, dto1, null, 0);
				
				if ("".equals(order) || order == null)
					throw new AppException("没有查询到符合条件的信息!");
				DBUtil.commit(con);
				HashMap<String, Object> retmap = new HashMap<String, Object>();
				retmap.put("beo", order);
				response.setBody(retmap);
			} catch (AppException ae) {
				response
						.setHead(ExceptionSupport(className, ae, request.getHead()));

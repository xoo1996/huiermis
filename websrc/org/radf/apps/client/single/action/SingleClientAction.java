...
import org.radf.apps.charge.action.ChargeAction;
...
//			}
			ClassHelper.copyProperties(scForm, sc);
			if("y".equals(scForm.getJqcx())){
				if(scForm.getIctnm()!=null&&scForm.getIctnm().length()>0){
					sc.setFileKey("clt01_001");
				}else{
					saveSuccessfulMsg(req, "精确查询用户姓名必填！");
					return mapping.findForward("backspace");
				}
			}else
...
/*			sc.setIctpro(sc.getIctpro()==null?"":sc.getIctpro().split(",")[0]);
			...
			//修改同时修改会员系统的联系方式
			if(tel!=""){
				String memberNo = scForm.getIctid();
				String sql = "SELECT icttel FROM TBLINDCLIENT where ictid = '"+memberNo+"'";
				Connection conn = DBUtil.getConnection();
				List result = (Vector) DBUtil.querySQL(conn,sql);
				String oldphone=(String)((HashMap)result.get(0)).get("1");
				conn.close();
				if(oldphone!=null&&oldphone.length()>13){
					String [] stringArr= oldphone.split(" ");
					oldphone=stringArr[0].trim();
				}//找出旧的手机号码
				
				String phone = tel.split(" ")[0];
				
				if(!phone.equals(oldphone)){//如果手机号码不同，则修改为新的号码
					String[] opAddEntryArgs = {oldphone,phone};
					String url  = GlobalNames.HUIERMALL_LINK;
					String method = "changePhone";
					ChargeAction.sendService(opAddEntryArgs, url, method);
				}
			}
...
//		scForm.setFctcdt(null);
	...
	public ActionForward enterAddSCTL(ActionMapping mapping, ActionForm form,
		...
		/*
		 * SingleClientForm scForm = (SingleClientForm) form;
		 * scForm.setFctcdt(null); return mapping.findForward("addSCTL");
		 */
		SingleClientForm sc = (SingleClientForm) form;
		/* sc.setBvassess("unstart"); */
		String bvno = sc.getIctid();
		...
		List result = (Vector) DBUtil
				.querySQL(
						con,
						"SELECT m.tmksid,p.pdtnm,m.tmkplr,a.foldt FROM tblmaking m LEFT OUTER JOIN tblproduct p ON p.pdtid=m.tmkpid LEFT OUTER JOIN tblfolio a ON a.folno=m.tmkfno WHERE tmkcltid="
								+ bvno + " ORDER BY a.foldt DESC");
		if (result.size() > 0) {
			Date dt1 = (Date) ((HashMap) result.get(0)).get("4");
			String ear1 = (String) ((HashMap) result.get(0)).get("3");
			...
			if (dt1 != null && result.size() > 1) {
				...
				if (dt2 != null && dt1.equals(dt2)) {
					if (ear1.equals("0")) {
						sc.setBvleftnum((String) ((HashMap) result.get(0))
								.get("1"));
						sc.setBvlefttype((String) ((HashMap) result.get(0))
								.get("2"));
						sc.setBvrightnum((String) ((HashMap) result.get(1))
								.get("1"));
						sc.setBvrighttype((String) ((HashMap) result.get(1))
								.get("2"));
					} else {
						sc.setBvrightnum((String) ((HashMap) result.get(0))
								.get("1"));
						sc.setBvrighttype((String) ((HashMap) result.get(0))
								.get("2"));
						sc.setBvleftnum((String) ((HashMap) result.get(1))
								.get("1"));
						sc.setBvlefttype((String) ((HashMap) result.get(1))
								.get("2"));
					}
				} else {
					if (ear1.equals("0")) {
 ...
 */
	 ...
	 * 
	 */
	public ActionForward queryDebug(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		SingleClient singleClient = new SingleClient();
		 SingleClientForm scForm = (SingleClientForm) form; 
		String ictsid = req.getParameter("ictsid");
		ActionForward af = new ActionForward();
		String forward = "/client/single/queryDebug.jsp";
		try {
			/*
			 * Connection conn=null; conn=DBUtil.getConnection(); String hql =
			 * "SELECT id,   usercode as ictid,   i.ictnm,   g.gctnm,   groupid as ictgctid,   filename,   uploadtime FROM tblfile f LEFT JOIN tblindclient i on f.usercode =i.ictid left join tblgrpclient g on f.groupid=g.gctid WHERE usercode='"
			 * + ictid + "'";
			 */
			String hql = "";
			ArrayList<DebugData> reslist = getResList(ictsid);
			req.setAttribute("reslist", reslist);
			af = super.init(req, forward, hql);
			// 检查是否存在？
			/*
			 * if (null == req.getAttribute(GlobalNames.QUERY_DATA)) { String
			 * msg = "该用户暂无调试记录！"; super.saveSuccessfulMsg(req, msg); }
			 */
			// 检查是否存在？
	...
	private ArrayList<DebugData> getResList(String nemid) {
		Connection con = null;
		ArrayList<DebugData> resList = new ArrayList<DebugData>();
		try {
			con = DBUtil.getConnection();
			String hql = "SELECT id,   usercode as jsid,  filename,   uploadtime FROM tblfile f  WHERE usercode='"
					+ nemid + "'";
			List result = (Vector) DBUtil.querySQL(con, hql);
			if (result.size() > 0) {
				for (int i = 0; i < result.size(); i++) {
					...
					debug.setId((a1 == null ? "" : a1.toString()));
					debug.setIctid((a2 == null ? "" : a2.toString()));
					debug.setFilename((a3 == null ? "" : a3.toString()));
					debug.setUploadtime((a4 == null ? "" : a4.toString()));
					/*

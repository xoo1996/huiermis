...
//			}
	...
	public ActionForward storageDetail(ActionMapping mapping,ActionForm form,
		...
		try {
			...
			String sql = "select sum(stoamount*stoactype) as total from (select t.* ,rownum as raa from ( select g.gctnm,s.stoid,s.stogrcliid, s.isrepair,s.stoproid,s.stoproname, s.stoamount, s.stoamountun, s.stoproprice, s.stodate, s.stooutdate,s.stoactype, s.storemark,s.stodisc,NVL2(c.pcllarge,c.pcllarge||'-'||c.pclmid||'-'||c.pclsmall,'') as stoprotype,p.pdtprc,p.pdtut from tblsto s left join tblgrpclient g on g.gctid=s.stogrcliid  left join tblproclass c on s.stoprotype=c.pclid  left join tblproduct p on s.stoproid=p.pdtid where 1=1 ";
						
						if(inStoreForm.getGctnm() != null && !inStoreForm.getGctnm().equals("")){
							sql += "and g.gctnm = '"+inStoreForm.getGctnm()+"' ";
						}
						if(inStoreForm.getStoproid() != null && !inStoreForm.getStoproid().equals("")){
							sql += "and s.stoproid = '"+inStoreForm.getStoproid()+"' ";
						}
						if(inStoreForm.getStoproname() != null && !inStoreForm.getStoproname().equals("")){
							sql += "and s.stoproname = '"+inStoreForm.getStoproname()+"' ";
						}
						if(inStoreForm.getIsrepair() != null && !inStoreForm.getIsrepair().equals("")){
							sql += "and s.isrepair = '"+inStoreForm.getIsrepair()+"' ";
						}
			sql += ") t ) where raa >= 1";
	...
	public ActionForward queryDetail(ActionMapping mapping,ActionForm form,
		...
		try {
			...
			if(sto.getGctnm() != null && !sto.getGctnm().equals("")){
				sql += "and g.gctnm = '"+sto.getGctnm()+"' ";
			}
			if(sto.getStoproid() != null && !sto.getStoproid().equals("")){
				sql += "and s.stoproid = '"+sto.getStoproid()+"' ";
			}
			if(sto.getStoproname() != null && !sto.getStoproname().equals("")){
				sql += "and s.stoproname = '"+sto.getStoproname()+"' ";
			}
			if(sto.getStoactype() != null && !sto.getStoactype().equals("")){
				sql += "and stoactype = '"+sto.getStoactype()+"' ";
			}
			if(sto.getStart() != null && !sto.getStart().equals("")){
				sql += "and stodate>=to_date('"+sto.getStart()+"','yyyy-MM-dd HH24:mi:ss')";
			}
			if(sto.getEnd() != null && !sto.getEnd().equals("")){
				sql += "and stodate<=to_date('"+sto.getEnd()+"','yyyy-MM-dd HH24:mi:ss')";
			}
sql += ") t ) where raa >= 1";

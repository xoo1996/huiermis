...
import org.radf.apps.business.form.BatForm;
...
import org.radf.apps.commons.entity.ReportFactory3;
import org.radf.apps.commons.entity.ReportFactoryOfBatCount;
import org.radf.apps.commons.entity.ReportOfBatCount;
...
public class BusinessAction extends ActionLeafSupport {
	 ...
	 * @throws Exception
	 */
	public ActionForward enterJM(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		LoginDTO dto = (LoginDTO)req.getSession().getAttribute("LoginDTO");
		String menuId = req.getParameter("menuId");
		// req.getSession().setAttribute("menuId", menuId);
		...
		bf.setIvtyear(now.get(Calendar.YEAR));
		bf.setIvtmonth(now.get(Calendar.MONTH)+1);
		bf.setActstamonth(now.get(Calendar.MONTH) + 1);
		ClassHelper.copyProperties(bf, form);
		return mapping.findForward(forward);
	}
	 ...
	 * @throws Exception
	 */
	public ActionForward toCheckPwd(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		LoginDTO dto = (LoginDTO)req.getSession().getAttribute("LoginDTO");
		String menuId = req.getParameter("menuId");
		String forward = menuId;
		Connection con = DBUtil.getConnection();
		String sql = "SELECT a.AAA102 from aa10 a where a.AAA100='PASSWORD' and a.AAA103='加盟店查询销账密码'";
		List result = (Vector) DBUtil.querySQL(con,sql);
		String password=((HashMap)result.get(0)).get("1").toString();
	 ...
	 *	ZSpreview.jsp月结查询
	 */
	public ActionForward findyuejie(ActionMapping actionMapping,
		...
		if(null!=mark&&mark.equals("back")){//如果后退的话，避免返回后因为gctid存在，返回页面只有一个门店数据
			String gctid=(String) req.getSession().getAttribute("ZZgctid");
			if("".equals(gctid)||gctid==null) bf.setActstagcltid(gctid);
			String gctnm=(String) req.getSession().getAttribute("ZZgctnm");
			if("".equals(gctnm)||gctnm==null) bf.setGctnm(gctnm);
			String area=(String) req.getSession().getAttribute("ZZarea");
			if(!("".equals(area)||area==null)) bf.setGctarea(area);
			String province=(String) req.getSession().getAttribute("ZZprovince");
			if(!("".equals(province)||province==null)) bf.setGctprovince(province);
			String gcttype=(String) req.getSession().getAttribute("ZZgcttype");
			if("".equals(gcttype)||gcttype==null) bf.setGcttype(gcttype);
			String actsta=(String) req.getSession().getAttribute("ZZactsta");
			if(!("".equals(actsta)||actsta==null)) bf.setActsta(actsta);
		}else{
			req.getSession().removeAttribute("ZZgctid");
			req.getSession().removeAttribute("ZZarea");
			req.getSession().removeAttribute("ZZprovince");
			req.getSession().removeAttribute("ZZgcttype");
			req.getSession().removeAttribute("ZZactsta");
			req.getSession().removeAttribute("ZZgctnm");
			req.getSession().setAttribute("ZZgctid", bf.getActstagcltid());
			req.getSession().setAttribute("ZZarea", bf.getGctarea());
			req.getSession().setAttribute("ZZprovince", bf.getGctprovince());
			req.getSession().setAttribute("ZZgcttype", bf.getGcttype());
			req.getSession().setAttribute("ZZactsta", bf.getActsta());
			req.getSession().setAttribute("ZZgctnm", bf.getGctnm());
		}
		try {
			...
			{
				bi.setFileKey("bus01_b009");//bus01005
				String hql = queryEnterprise(bi);
				...
				if(null!=bi.getGctnm()&&!"".equals(bi.getGctnm()))
				{
					hql+=" and g.gctnm='"+bi.getGctnm()+"'";
				}
				if(null!=bi.getGcttype()&&!"".equals(bi.getGcttype()))
				{
					hql+=" and g.gcttype='"+bi.getGcttype()+"'";
				}
				if(null!=bi.getGctprovince()&&!"".equals(bi.getGctprovince()))
				{
					hql+=" and g.gctprovince='"+bi.getGctprovince()+"'";
				}
...
//			}
	...
	/**
	 * ZSpreview.jsp  report月结详细查询
	 */
	public ActionForward yuejieEdit(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		String mark = req.getParameter("mark");
		Business bi = new Business();
		BusinessForm bf = (BusinessForm) actionForm;
		ActionForward af = new ActionForward();
		String forward1 = "/business/xiaozhang1.jsp";
		String forward2 = "/business/ZSxiaozhangEdit.jsp";
		Connection conn = null;
		String totalYSK = "0";
		try {
			ClassHelper.copyProperties(bf, bi);
			LoginDTO dto = (LoginDTO)req.getSession().getAttribute("LoginDTO");
			if("jiameng".equals(mark)||!("C".equals(bi.getGcttype())||"A".equals(bi.getGcttype()))){//不为直属店=加盟店？
				bi.setIvtyear(bi.getActstayear());
				bi.setIvtmonth(bi.getActstamonth());
				bi.setIvtgcltid(bi.getActstagcltid());
				//bi.setFileKey("bus01_010");
			...
			else if("zhishu".equals(mark)||("C".equals(bi.getGcttype())||"A".equals(bi.getGcttype()))){
				String hql="";
				if("".equals(bi.getIvttype()) || null==bi.getIvttype())
				{
					if("".equals(bi.getPdtnm()) || null==bi.getPdtnm())
	 ...
	 * 直属月结查询for edit页面
	 */
	public ActionForward ZSyuejiequery1(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		String mark = req.getParameter("mark");
		Business bi = new Business();
				...
				if ("normal".equals(bi.getIvttype())) {
					...
					if ("makenormal".equals(bi.getIvttype())) {
						...
						if ("".equals(bi.getPdtnm()) || null == bi.getPdtnm()) {
							...
							hql1 = hql1 + " and p.pdtnm like '%"
									+ bi.getPdtnm() + "%'";
							hql2 = hql2 + " and p.pdtnm like '%"
									+ bi.getPdtnm() + "%'";
						}
						hql = hql1 + " union all " + hql2;
					} else {
						...
						if ("makeEar".equals(bi.getIvttype())) {
							hql = hql + " and y.ivttype in('" + bi.getIvttype()
									+ "','makeEarYN') ";
						} else {
							hql = hql + " and y.ivttype='" + bi.getIvttype()
									+ "'";
	 ...
	 * 加盟店系统页面月结查询
	 */
	public ActionForward JMyuejiequery1(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		String mark = req.getParameter("mark");
		Business bi = new Business();
		...
		try {
			...
			hql = hql + " order by ivttype,ivtgcltid,ivtpdtid,ivtcltnm";
			String forward1 = "/business/JMxiaozhangEdit.jsp";
			String forward2 = "/business/JMxiaozhangQuery.jsp";
			if(mark.equals("edit")){
				af = super.init(req, forward1, hql, "1");
	...
	/**
	 * 加盟店系统页面月结查询JMxiaozhangEdit.jsp
	 */
	public ActionForward JMyuejiebackup(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		Business bi = new Business();
		BusinessForm bf = (BusinessForm) actionForm;
		...
		try {
			...
			if ("".equals(bi.getIvttype()) || null == bi.getIvttype()) {
				if ("".equals(bi.getPdtnm()) || null == bi.getPdtnm()) {
					bi.setFileKey("bus01_010j");
					hql = queryEnterprise(bi);
				} else {
					bi.setFileKey("bus01_015j");
					String hql1 = queryEnterprise(bi);
					hql1 = hql1 + " and p.pdtnm like '%" + bi.getPdtnm() + "%'";
					bi.setFileKey("bus01_016j");
					String hql2 = queryEnterprise(bi);
					hql2 = hql2 + " and p.pdtnm like '%" + bi.getPdtnm() + "%'";
					hql = hql1 + " union all " + hql2;
				}
			...
			conn = DBUtil.getConnection();
			Statement pstmt = conn.createStatement();
			String sql = "select sum(ivtpamnt) as totalYSK from tblinventoryjiameng where ivtyear='"
					+ bf.getIvtyear()
					+ "' and ivtmonth='"
					+ bf.getIvtmonth()
					+ "' and ivtgcltid='" + bf.getIvtgcltid() + "'";
	...
	public ActionForward ZSyuejiequery(ActionMapping actionMapping,
		...
		String mark=req.getParameter("mark");
		if(null!=mark&&mark.equals("back")){//如果后退的话，避免返回后因为gctid存在，返回页面只有一个门店数据
			String gctid=(String) req.getSession().getAttribute("ZSgctid");
			if("".equals(gctid)||gctid==null) bf.setIvtgcltid(null);
			String area=(String) req.getSession().getAttribute("ZSarea");
			if(!("".equals(area)||area==null)) bf.setGctarea(area);
			String province=(String) req.getSession().getAttribute("ZSprovince");
			if(!("".equals(province)||province==null)) bf.setGctprovince(province);
		}else{
			req.getSession().removeAttribute("ZSgctid");
			req.getSession().removeAttribute("ZSarea");
			req.getSession().removeAttribute("ZSprovince");
			req.getSession().setAttribute("ZSgctid", bf.getIvtgcltid());
			req.getSession().setAttribute("ZSarea", bf.getGctarea());
			req.getSession().setAttribute("ZSprovince", bf.getGctprovince());
		}
		try {
			ClassHelper.copyProperties(bf, bi);
	...
	public ActionForward JMyuejiequery(ActionMapping actionMapping,
		...
		try {
			...
			else
			{
				...
				hql+=" group by y.ivtgcltid,y.ivtyear,y.ivtmonth) x,tblgrpclient g left outer join tblactsta t on g.gctid = t.actstagcltid where x.ivtgcltid=t.actstagcltid and exists (select gctid from tblgrpclient p where (gcttype!='C' and gcttype!='A') and gctvalid!='0'and p.gctid=t.actstagcltid)";//and not exists (select gctid from tblgrpclient p where gcttype='A' and gctvalid='0'and p.gctid=t.actstagcltid
	 ...
	 * @throws Exception
	 */
	public ActionForward copyYuejie(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		String mark = (String)req.getParameter("mark");
		Business bi = new Business();
		...
		try {
			//conn = DBUtil.getConnection();
			//改月是否已经月结
			/*String sql = "select count(*)  from tblinventory i where i.ivtyear=" + bf.getIvtyear() + " and i.ivtmonth=" + bf.getIvtmonth();
			List result = (Vector) DBUtil.querySQL(conn,sql);
			String count=((HashMap)result.get(0)).get("1").toString();
			String sql1 = "select count(*)  from tblinventoryjiameng i where i.ivtyear=" + bf.getIvtyear() + " and i.ivtmonth=" + bf.getIvtmonth();
			List result1 = (Vector) DBUtil.querySQL(conn,sql1);
			String count1=((HashMap)result1.get(0)).get("1").toString();
			if(!(Integer.parseInt(count)>0)){
				String msg = "请先完成该月的结算！";
				super.saveSuccessfulMsg(req, msg);
			}else if(Integer.parseInt(count1)>0){
				String msg = "该月已经进行过该操作！";
				super.saveSuccessfulMsg(req, msg);
			}else{
				
			}*/
			if("redo".equals(mark)){//重做该月
				long startTime = System.currentTimeMillis();
				long endTime = System.currentTimeMillis();
				con = DBUtil.getConnection();
				DBUtil.beginTrans(con);
				String ss = "delete /*+parallel(4)*/ from TBLINVENTORYJIAMENG j where j.ivtmonth="+bf.getIvtmonth()+" and j.ivtyear="+bf.getIvtyear()+"";
				DBUtil.execSQL(con, ss);
				String s = "insert /*+parallel(4)*/ into TBLINVENTORYJIAMENG (SELECT * from TBLINVENTORY i " +
						"where i.ivtmonth="+bf.getIvtmonth()+" and i.ivtyear="+bf.getIvtyear()+")";
				System.out.println(s);
				System.out.println(ss);
				DBUtil.execSQL(con, s);
				DBUtil.commit(con);
				System.out.println("time: " + (endTime - startTime) / 1000 + " s");
				String time = "操作成功，耗时："+ (endTime - startTime) / 1000 + " s";
				super.saveSuccessfulMsg(req, time);
			}else{
				long startTime = System.currentTimeMillis();
				long endTime = System.currentTimeMillis();
		...
		return actionMapping.findForward("preXiaozhang");
	}
	
	public ActionForward preXiaozhang(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		BusinessForm bf = (BusinessForm) actionForm;
		Connection conn = null;
		bf.setIvtyear(Integer.parseInt(req.getParameter("year")));
		bf.setIvtmonth(Integer.parseInt(req.getParameter("month")));
		try {
			conn = DBUtil.getConnection();
			//改月是否已经月结
			String sql = "select count(*)  from tblinventory i where i.ivtyear=" + bf.getIvtyear() + " and i.ivtmonth=" + bf.getIvtmonth();
			...
			if (!(Integer.parseInt(count)>0)) {
				res.setCharacterEncoding("GBK");
				res.getWriter().write("[{sign:'0'}]");
			} else if(Integer.parseInt(count1)>0){
				res.setCharacterEncoding("GBK");
				res.getWriter().write("[{sign:'1'}]");
			}else{
				res.setCharacterEncoding("GBK");
				res.getWriter().write("[{sign:'2'}]");
			}
		} catch (Exception e) {
			this.saveErrors(req, e);
		} finally {
	...
	public ActionForward batchSubmit(ActionMapping mapping, ActionForm form,
		...
		//该月是否已经备份月结数据到tblinventoryjiameng
		BusinessForm bf = (BusinessForm) form;
		Connection conn = DBUtil.getConnection();
		String sql1 = "select count(*)  from tblinventoryjiameng i where i.ivtyear=" + bf.getIvtyear() + " and i.ivtmonth=" + bf.getIvtmonth();
		List result1 = (Vector) DBUtil.querySQL(conn,sql1);
		String count=((HashMap)result1.get(0)).get("1").toString();
		if(!(Integer.parseInt(count)>0)){
			super.saveSuccessfulMsg(req, "该月销账预处理还未进行！");
	 ...
	 * @throws Exception
	 */
	public ActionForward batchSubmitOnly(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		String mark=req.getParameter("mark");
		String[] chk = req.getParameterValues("chk");
		Collection<Business> collection = null;
		...
		try {
			...
			HashMap<String, Object> mapRequest = new HashMap<String, Object>();
			mapRequest.put("userId", dto1.getBsc011());
			mapRequest.put("mark", mark);
			mapRequest.put("collection", checked);
			requestEnvelop.setBody(mapRequest);
			// 调用对应的Facade业务处理方法
			ResponseEnvelop resEnv = businessFacade.modifyOnly(requestEnvelop);
			// 处理返回结果
			returnValue = processRevt(resEnv);
			if (returnValue.isSucessFlag()) {
				super.saveSuccessfulMsg(req, "批量保存信息成功!");
...
//          PrintWriter   out   =   res.getWriter();   
	...
	public ActionForward queryqyxs(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			Connection con = DBUtil.getConnection();
			String sql = "select nvl(sum(i.ivtpamnt),0) as temp02 from tblinventory i where i.ivtyear=" + bf.getIvtyear() + " and i.ivtmonth=" + bf.getIvtmonth();
			List result = (Vector) DBUtil.querySQL(con,sql);
			String count=((HashMap)result.get(0)).get("1").toString();
			req.getSession().setAttribute("count", count);
			af = init(req, forward, queryhql,18, "1");
	...
	public ActionForward salequery(ActionMapping mapping,
		...
		try {
			...
			String sql1 = "select * from (select distinct i.ivtgcltid, g.gctnm,g.gcttype,g.gctprovince,g.gctarea,p.pdtprc,p.pdtnm,p.pdttype ";
	 ...
	 * Time：2017/08/04
	 */
	public ActionForward batCount(ActionMapping mapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {

		String forward = "/business/batCount.jsp";
		ActionForward af = new ActionForward();
		Business bs = new Business();
		BusinessForm bsForm = (BusinessForm) actionForm;
		
		Integer yearStart = bsForm.getIvtyear();
		Integer yearEnd = bsForm.getIvtyearEnd();
		Integer monthStart = bsForm.getIvtmonths();
		Integer monthEnd = bsForm.getIvtmontht();
		String ivtgcltid = bsForm.getIvtgcltid();//门店
		String area = bsForm.getGctarea();//所属区域
		String bb = bsForm.getBb();//产品品牌
		String bm = bsForm.getBm();//商品类型
		
		try {
			ClassHelper.copyProperties(bsForm, bs);
			 String sql1="select distinct s.gctnm,s.gctarea,b.BRAND as bb,b.NAME as bmresult,sum(m.battery_qty)over(PARTITION BY m.battery_type,m.STORE_NO order by m.battery_type) as batcountnum from MEM_BAT m LEFT JOIN STORE s on s.GCTID = m.STORE_NO LEFT JOIN BATTERY b on b.TYPE = m.BATTERY_TYPE where b.type != '0' and m.status != 'del' and m.DATA between trunc(add_months(to_date('"+yearStart+"' ||'-' ||'"+monthStart+"' ||'-' ||'1','yyyy-mm-dd'),-1),'mm')+25 AND to_date('"+yearEnd+"' ||'-' ||'"+monthEnd+"' ||'-' ||'1','yyyy-mm-dd')+24 ";
			 String sql2="ORDER BY s.gctnm,s.GCTAREA ";
			
			if (ivtgcltid != null && !"".equals(ivtgcltid)) {
				sql1 += "and s.gctid='"+ivtgcltid+"' ";
			}
			
			if (area != null && !"".equals(area)) {
				sql1 += "and s.gctarea='"+area+"' ";
			}
...
/*			req.setAttribute("flagDB", "newOracle");*/
	 ...
	 * 优惠收费查询
	 */
	public ActionForward youhuiquery(ActionMapping mapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
	/*	SingleClient singleClient=new SingleClient();
		...
		return af;*/
		String forward = "/business/youhuiquery.jsp";
		ActionForward af = new ActionForward();
		BusinessForm bsForm = (BusinessForm) actionForm;
		Business bs = new Business();
		...
		try {
			ClassHelper.copyProperties(bsForm, bs);
			bs.setFileKey("youhui_query");
			String hql=queryEnterprise(bs);
			af=super.init(req, forward, hql);
			if(null==req.getAttribute(GlobalNames.QUERY_DATA)){
				String msg="没有符合条件的记录";
				super.saveSuccessfulMsg(req, msg);
	...
	public ActionForward profit(ActionMapping actionMapping,
		...
		Map<String, Object> parameters = new HashMap<String, Object>();
		try {
			ReportFactory2 pf = new ReportFactory2();
...
//			ouputStream.close();
	 ...
	 * 利润表 数量5位 单价
	 */
	public ActionForward profitNew(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		...
		Map<String, Object> parameters = new HashMap<String, Object>();
		try {
			ReportFactory3 pf = new ReportFactory3();
			List<Report> data = pf.setCollection(ivtgcltid, year, month);
			JRDataSource dataSource = new JRBeanCollectionDataSource(data);
			...
			parameters.put("month", month);
			File reportFile = new File(req.getSession().getServletContext()
					.getRealPath("\\WEB-INF\\report\\profit_bean_n.jasper"));
			ServletOutputStream ouputStream = res.getOutputStream();
            String reportclass = "profit";
			res.setContentType("application/vnd.ms-excel"); 
			res.setHeader("content-disposition", 
...
//            "客户信息表.xls");   
	 ...
	 * 生成库存超期数据
	 */
	public ActionForward calcuExp(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception{
		Business business = new Business();
		BusinessForm bf = (BusinessForm)form;
		Connection con = DBUtil.getConnection();;
		...
		try{
			...
			List result6 = (Vector) DBUtil.querySQL(con,
					"select gctstate from tblgrpclient");
			String gctstate=(String) ((HashMap) result6.get(0)).get("1");
			if ("0".equals(gctstate)) {
				saveSuccessfulMsg(req, "计算正在进行中，请等待！");
				return mapping.findForward("backspace");
			}
			BusinessFacade facade = (BusinessFacade)getService("BusinessFacade");
			RequestEnvelop requestEnvelop = new RequestEnvelop();
			...
			requestEnvelop.setBody(mapRequest);
			// 调用对应的Facade业务处理方法
			ResponseEnvelop resEnv = facade.gctstate(requestEnvelop);
			// 处理返回结果
			returnValue = processRevt(resEnv);
			if(returnValue.isSucessFlag())
			{
	 ...
	 * 保存修改的库存期限
	 */
	public ActionForward staSource(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)throws Exception{
		Business business = new Business();
		BusinessForm bf = (BusinessForm)form;
		try{
			...
			business.setBsc011(dto1.getBsc011());
			//select gctarea from tblgrpclient where gctid=:bsc011!
			Connection conn = DBUtil.getConnection();
			String sql = "select gctarea from tblgrpclient where gctid='" + dto1.getBsc011() + "'";
			List result = (Vector) DBUtil.querySQL(conn, sql);
			String gctarea=((HashMap)result.get(0)).get("1").toString();
			business.setGctarea(gctarea);
			//close oracle con
			DBUtil.closeConnection(conn);
			BusinessFacade facade = (BusinessFacade) getService("BusinessFacade");
			RequestEnvelop requestEnvelop = new RequestEnvelop();
			...
			requestEnvelop.setBody(mapRequest);
			// 调用对应的Facade业务处理方法
			ResponseEnvelop resEnv = facade.saveWeb(requestEnvelop);
			// 处理返回结果
			returnValue = processRevt(resEnv);
			if (returnValue.isSucessFlag()) {
				super.saveSuccessfulMsg(request, "提交网络销售来源数据成功！");
			} else {
				String[] aa = StringUtil.getAsStringArray(returnValue.getMsg(),
						"|");
				super.saveErrors(request, new AppException(aa[3]));
		...
		return mapping.findForward("staSouDet");
	}
	
	public ActionForward staquery(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		Business bi = new Business();
		BusinessForm bf = (BusinessForm) actionForm;
		ActionForward af = new ActionForward();
		String forward = "/business/staquery.jsp";
		try {
			ClassHelper.copyProperties(bf, bi);
			//Connection con = DBUtil.getConnection();
			if("no".equals(bf.getWebcommit())){
				bi.setFileKey("web_except");
			}else
				bi.setFileKey("web_select");
			String hql = queryEnterprise(bi);
			af = super.init(req, forward, hql);
			if (null == req.getAttribute(GlobalNames.QUERY_DATA)) {
				String msg = "没有查询到符合条件的记录！";
	...
	public ActionForward queryWebSell(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		Business bi = new Business();
		BusinessForm bf = (BusinessForm) actionForm;
		...
		String forward = "/business/queryWebSell.jsp";
		try {
			LoginDTO dto = (LoginDTO) req.getSession().getAttribute("LoginDTO");
			ClassHelper.copyProperties(bf, bi);
			Connection con = DBUtil.getConnection();
			bi.setBsc011(dto.getBsc011());
			if("t".equals(tp)){
				bi.setIvtmonth(bi.getWebmonth());
				bi.setIvtyear(bi.getWebyear());
				String sql = "select bsc011 from sc05 where bsc012='"+ bi.getGctnm() + "'";
				List result = (Vector) DBUtil.querySQL(con, sql);
				if (result.size() > 0) {
					String bsc011 = ((HashMap) result.get(0)).get("1").toString();
					bi.setBsc011(bsc011);
				}
			}
			//close oracle con
			DBUtil.closeConnection(con);
			bi.setFileKey("web_query");
			String hql = queryEnterprise(bi);
			af = super.init(req, forward, hql);
			if (null == req.getAttribute(GlobalNames.QUERY_DATA)) {
				String msg = "没有查询到符合条件的记录！";
	 ...
	 * 跳转网络销售信息
	 */
	public ActionForward modifyWebSell(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception{
		Business business = new Business();
		BusinessForm bf = (BusinessForm)form;
		...
		try{
			...
			requestEnvelop.setBody(mapRequest);
			// 调用对应的Facade业务处理方法
			ResponseEnvelop resEnv = facade.printWeb(requestEnvelop);
			// 处理返回结果
			returnValue = processRevt(resEnv);
			if(returnValue.isSucessFlag())
			{
				List listci = (ArrayList) ((HashMap) resEnv.getBody())
						.get("beo");// 配置项信息
				req.getSession().setAttribute("listci", listci);
				ClassHelper.copyProperties(listci.get(0), bf);
				return mapping.findForward("webmodify");
			}
			else {
				String[] aa = StringUtil.getAsStringArray(returnValue.getMsg(), "|");
				super.saveSuccessfulMsg(req, aa[3]);
	 ...
	 * 跳转网络销售信息
	 */
	public ActionForward updateWebSell(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception{
		Business business = new Business();
		BusinessForm bf = (BusinessForm)form;
		...
		try{
			...
			requestEnvelop.setBody(mapRequest);
			// 调用对应的Facade业务处理方法
			ResponseEnvelop resEnv = facade.modifyWeb(requestEnvelop);
			// 处理返回结果
			returnValue = processRevt(resEnv);
			if(returnValue.isSucessFlag())
			{
				super.saveSuccessfulMsg(req, "修改网络销售信息成功!");
				return mapping.findForward("staSouDet");
			}
			else {
				String[] aa = StringUtil.getAsStringArray(returnValue.getMsg(), "|");
	...
	public ActionForward inDetail(ActionMapping mapping, ActionForm form,
		...
		Business bus = new Business();
		ActionForward af = new ActionForward();
		String forward = "/business/webSellSave.jsp";
		String type=req.getParameter("type");
		if("cvt".equals(type)){
			forward = "/business/webCvtSave.jsp";
		}
		try {
			bus.setFileKey("to_amotize");
			String hql = queryEnterprise(bus);
			af = super.init(req, forward, hql);
		} catch (AppException ex) {
			this.saveErrors(req, ex);
		} catch (Exception e) {
	 ...
	 * 保存本月本店网络销售明细
	 */
	public ActionForward saveWebSell(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		BusinessForm bf = (BusinessForm) form;
		String tp = req.getParameter("tp");
		SubmitDataMap data = new SubmitDataMap(req);
		String[] wnmList = data.getParameterValues("webname"); // 
		String[] pnmList = data.getParameterValues("pdtnm"); // 
		String[] numList = data.getParameterValues("webnum");// 
		String[] monList = data.getParameterValues("webmoney"); // 
		String[] calList = data.getParameterValues("webcall"); // 
		String[] souList = data.getParameterValues("webfrom"); // 
		String[] intList = data.getParameterValues("webintro"); // 
		try {
			LoginDTO dto = (LoginDTO) req.getSession().getAttribute("LoginDTO");
			int size = wnmList.length;
			List<Business> bsList = new Vector<Business>();
			Calendar cal = Calendar.getInstance();
			int year = cal.get(Calendar.YEAR);
			int month = cal.get(Calendar.MONTH )+1;
			for (int i = 0; i < size; i++) {
				Business bs = new Business();
				bs.setWebname(wnmList[i]);
				bs.setPdtnm(pnmList[i]);
				bs.setWebnum(Integer.parseInt(numList[i]));
				bs.setWebmoney(Double.parseDouble(monList[i])); // 
				bs.setWebcall(calList[i]);
				bs.setBsc011(dto.getBsc011());
				bs.setWebyear(year);
				bs.setWebmonth(month);
				bsList.add(bs);
				if (tp.equals("s")) {
					bs.setWebfrom(souList[i]);
					bs.setWebsc("s");
				} else {
					bs.setWebintro(intList[i]);
					bs.setWebsc("c");
				}
			}
			ResponseEnvelop resEnv = null;
			...
			// 将Application对象放入HashMap
			HashMap<String, Object> mapRequest = new HashMap<String, Object>();
			mapRequest.put("bsList", bsList);
			requestEnvelop.setBody(mapRequest);
			// 调用对应的Facade业务处理方法
			resEnv = businessFacade.commit(requestEnvelop);
			// 处理返回结果
			returnValue = processRevt(resEnv);
			if (returnValue.isSucessFlag()) {
				if (tp.equals("s")) {
					super.saveSuccessfulMsg(req, "本店本月网络销售明细提交成功！" );
				} else {
					super.saveSuccessfulMsg(req, "网络来源转至其他惠耳店明细提交成功！" );
				}
				return go2Page(req, mapping, "business");
			} else {
				String[] aa = StringUtil.getAsStringArray(returnValue.getMsg(),

...
public class FeeAction extends ActionLeafSupport {
	 ...
	 * @throws Exception
	 */
	public ActionForward saveFee(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		Fee fee = new Fee();
		FeeForm fef = (FeeForm) form;
		try {
			...
			fee.setFeeop(dto1.getBsc011());
			fee.setFeegctid(dto1.getBsc011());
			fee.setFeegctname(dto1.getBsc012());
			fee.setBsc012(dto1.getBsc012());
			fee.setFeeopdata(DateUtil.getSystemCurrentTime());
			//重新计算本月销售额，商务，合计汇入，本月结余，前端传不过来值，手动计算。。
			fee.setRfeesales(fee.getRfeehuier()+fee.getRfeeweiting()+fee.getRfeeqie()+fee.getRfeeparts()+fee.getRfeerepair()+fee.getRfeeelse()+fee.getRfeeearmould()-fee.getRfeebenefitm());
			fee.setRfeebusiness(fee.getRfeesales()-fee.getRfeefinance()-fee.getRfeecollection());
			fee.setRfeetotalin(fee.getRfeebusiness()+fee.getRfeeelin()+fee.getRfeewagein()-fee.getRfeebenefitin());
			fee.setRfeesurplus(fee.getRfeepresurplus()+fee.getRfeebusin()-fee.getRfeerealpay());
			BusinessFacade facade = (BusinessFacade) getService("BusinessFacade");
			RequestEnvelop requestEnvelop = new RequestEnvelop();
			EventResponse returnValue = new EventResponse();
			// 将Application对象放入HashMap
			...
			if (returnValue.isSucessFlag()) {
				...
				String workString = (String) ((HashMap) resEnv.getBody())
						.get("workString");
				FindLog.insertLog(req, gctid, workString);
				return mapping.findForward("insertfee1");
			} else {
				String[] aa = StringUtil.getAsStringArray(returnValue.getMsg(),
						"|");
				super.saveSuccessfulMsg(req, aa[3]);
	 ...
	 * @throws Exception
	 */
	public ActionForward submitFee(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		Fee fee = new Fee();
		FeeForm fef = (FeeForm) form;
		try {
			...
			requestEnvelop.setBody(mapRequest);
			// 调用对应的Facade业务处理方法
			ResponseEnvelop resEnv = facade.submit(requestEnvelop);
			// 处理返回结果
			returnValue = processRevt(resEnv);
			if (returnValue.isSucessFlag()) {
	...
	public ActionForward findData(ActionMapping mapping, ActionForm form,
		...
		requestEnvelop.setBody(mapRequest);
		// 调用对应的Facade业务处理方法
		ResponseEnvelop resEnv = facade.queryMonthReport(requestEnvelop);
		// 处理返回结果
		returnValue = processRevt(resEnv);
		if (returnValue.isSucessFlag()) {
	 ...
	 * 未提交数据查询()
	 */
	public ActionForward queryNoSubmit(ActionMapping mapping, ActionForm form,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		Fee fee = new Fee();
		FeeForm ff = (FeeForm) form;
		ActionForward af = new ActionForward();
		// String forward = "/business/jasper/costquery.jsp";
		String forward = "/business/monthSummary/queryNoSubmit.jsp";
		try {
			ClassHelper.copyProperties(ff, fee);
			if(null==fee.getGctarea()||fee.getGctarea().equals(""))
				fee.setFileKey("fee_selectno");
			else
				fee.setFileKey("fee_selectno1");
	...
	public ActionForward report(ActionMapping actionMapping,
		...
		try {
			...
			JasperPrint rptpnt = JasperFillManager.fillReport(
					reportFile.getPath(), parameters, conn1);
	...
	public ActionForward exportFee(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		FeeForm bf = (FeeForm) actionForm;
		Connection conn1 = null;
		try {
				...
				if (gctarea == null || "".equals(gctarea)) {
					reportFile = new File(
							req.getSession()
									.getServletContext()
									.getRealPath(
											"\\WEB-INF\\report\\feesheet_1.jasper"));// 客户名称为空，区域为空
				} else {
					reportFile = new File(
							req.getSession()
									.getServletContext()
									.getRealPath(
											"\\WEB-INF\\report\\feesheet_2.jasper"));// 客户名称为空，区域不为空
					parameters.put("gctarea", gctarea);
				}
			} else {
				reportFile = new File(req.getSession().getServletContext()
						.getRealPath("\\WEB-INF\\report\\feesheet_3.jasper"));// 客户名称不为空
				parameters.put("gctid", gctid);
			}
			String webRootPath = req.getSession().getServletContext()
					.getRealPath("/");
			parameters.put("startYear", bf.getStartyear());
			...
			exporter.setParameter(JRXlsExporterParameter.IS_DETECT_CELL_TYPE,
					Boolean.TRUE);// 设置检测单元格格式，解决“excel以文本显示数字”
			exporter.setParameter(JRExporterParameter.JASPER_PRINT, rptpnt);
			exporter.setParameter(JRExporterParameter.OUTPUT_STREAM,
					ouputStream);
			// exporter.setParameter(JRExporterParameter.OUTPUT_FILE_NAME,
	 ...
	 * 
	 */
	public ActionForward report1(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		FeeForm bf = (FeeForm) actionForm;
		Connection conn1 = null;
		try {
			...
			// "Name"是报表中定义过的一个参数名称,其类型为String 型
			reportFile = new File(req.getSession().getServletContext()
					.getRealPath("\\WEB-INF\\report\\feereport111.jasper"));
			parameters.put("year", bf.getFeeyear());
			parameters.put("month", bf.getFeemonth());
			parameters.put("gctid", bf.getFeegctid());
			// 连接到数据库
			conn1 = DBUtil.getConnection();
			String update_sql = "UPDATE tblfee t SET isprint=1 WHERE t.feeyear = '"+bf.getFeeyear()+"' and t.feemonth='"+bf.getFeemonth()+"' and t.feegctid='"+bf.getFeegctid()+"' ";
			DBUtil.execSQL(conn1, update_sql);
			// 在控制台显示一下报表文件的物理路径
			// System.out.println(reportFile.getPath());
	...
	@SuppressWarnings("deprecation")
	public ActionForward companyfee(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		FeeForm bf = (FeeForm) actionForm;
		Connection conn1 = null;
		...
		try {
			...
			if (gctid == null || "".equals(gctid)) {
				if (gctarea == null || "".equals(gctarea)) {
					reportFile = new File(req
							.getSession()
							.getServletContext()
							.getRealPath(
									"\\WEB-INF\\report\\companyfee_4.jasper"));// 客户名称为空，区域为空
				} else {
					reportFile = new File(req
							.getSession()
							.getServletContext()
							.getRealPath(
									"\\WEB-INF\\report\\companyfee_3.jasper"));// 客户名称为空，区域不为空
					parameters.put("gctarea", gctarea);
				}
			} else {
				reportFile = new File(req.getSession().getServletContext()
			...
			// String sql1 =
			// "select b.aaa103 from aa10 b left outer join tblgrpclient g on g.gctarea=b.aaa102 where b.aaa100='GCTAREA' and g.gctid='"
			// + feegctid + "'";
			// String sql2 =
			// "select b.aaa103 from aa10 b left outer join tblgrpclient g on g.gctprovince=b.aaa102 where b.aaa100='GCTPROVINCE' and g.gctid='"
			// + feegctid + "'";
			// String sql3 =
			// "select b.aaa103 from aa10 b left outer join tblgrpclient g on g.gcttype=b.aaa102 where b.aaa100='GCTTYPE' and g.gctid='"
			// + feegctid + "'";
	...
	//增加新按钮做新的优惠费用打印
	@SuppressWarnings("deprecation")
	public ActionForward companyfeeNew(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		FeeForm bf = (FeeForm) actionForm;
		Connection conn1 = null;
			...
			if (gctid == null || "".equals(gctid)) {
				if (gctarea == null || "".equals(gctarea)) {
					reportFile = new File(req
							...
							.getServletContext()
							.getRealPath(
									"\\WEB-INF\\report\\companyfee_4n.jasper"));// 客户名称为空，区域为空
				} else {
					reportFile = new File(req
							.getSession()
							.getServletContext()
							.getRealPath(
									"\\WEB-INF\\report\\companyfee_3n.jasper"));// 客户名称为空，区域不为空
					parameters.put("gctarea", gctarea);
				}
			} else {
				reportFile = new File(req.getSession().getServletContext()
						.getRealPath("\\WEB-INF\\report\\companyfee_5n.jasper"));// 客户名称不为空
				parameters.put("gctid", gctid);
			}
			String webRootPath = req.getSession().getServletContext()
					.getRealPath("/");
	...
	@SuppressWarnings("deprecation")
	public ActionForward printfee(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		FeeForm bf = (FeeForm) actionForm;
		Connection conn1 = null;
			...
			if (gctid == null || "".equals(gctid)) {
				if (gctarea == null || "".equals(gctarea)) {
					reportFile = new File(req
							...
							.getServletContext()
							.getRealPath(
									"\\WEB-INF\\report\\companyfee_4s.jasper"));// 客户名称为空，区域为空
				} else {
					reportFile = new File(req
							.getSession()
							.getServletContext()
							.getRealPath(
									"\\WEB-INF\\report\\companyfee_3s.jasper"));// 客户名称为空，区域不为空
					parameters.put("gctarea", gctarea);
				}
			} else {
				reportFile = new File(req.getSession().getServletContext()
						.getRealPath("\\WEB-INF\\report\\companyfee_5s.jasper"));// 客户名称不为空
				parameters.put("gctid", gctid);
			}
			String webRootPath = req.getSession().getServletContext()
					.getRealPath("/");
	 ...
	 * 查询月度汇总综合费用
	 */
	public ActionForward query1(ActionMapping actionMapping,
			ActionForm actionForm, HttpServletRequest req,
			HttpServletResponse res) throws Exception {
		Fee fee = new Fee();
		FeeForm ff = (FeeForm) actionForm;
		ActionForward af = new ActionForward();
		// String forward = "/business/jasper/costquery.jsp";
		String forward = "/business/monthSummary/query1.jsp";
		try {
			ClassHelper.copyProperties(ff, fee);
			fee.setFileKey("fee_select1");
			String hql = queryEnterprise(fee);
			af = init(req, forward, hql);
			if (null == req.getAttribute(GlobalNames.QUERY_DATA)) {
				String msg = "这个月费用信息还没录入！";
	...
	public ActionForward edit1(ActionMapping mapping, ActionForm form,
		...
		requestEnvelop.setBody(mapRequest);
		// 调用对应的Facade业务处理方法
		ResponseEnvelop resEnv = facade.query1(requestEnvelop);
		// 处理返回结果
		returnValue = processRevt(resEnv);
		if (returnValue.isSucessFlag()) {
			HashMap mapResponse = (HashMap) resEnv.getBody();
			...
			ClassHelper.copyProperties(listci.get(0), ff);
			return mapping.findForward("savefee1");
	...
	public ActionForward grant(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			requestEnvelop.setBody(mapRequest);
			// 调用对应的Facade业务处理方法
			ResponseEnvelop resEnv = facade.grant(requestEnvelop);
			// 处理返回结果
			returnValue = processRevt(resEnv);
			if (returnValue.isSucessFlag()) {
				super.saveSuccessfulMsg(req, "授权成功!");
				// return mapping.findForward("backspace");
	...
	public ActionForward getLastMonth(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (returnValue.isSucessFlag()) {
				...
				res.getWriter().write(
						"[{feedevice:" + fee.getFeedevice() + ",feepension:"
								+ fee.getFeepension() + ",feeassets:"
								+ fee.getFeeassets() + ",feebuilt:"
								+ fee.getFeebuilt() + ",feeopen:"
								+ fee.getFeeopen() + ",feerent:"
								+ fee.getFeerent() + ",feetrans:"
								+ fee.getFeetrans() + "}]");
			}
		} catch (Exception e) {
			super.saveErrors(req, e);
	...
	public ActionForward setAmortize(ActionMapping mapping, ActionForm form,
		...
		String feegctid = req.getParameter("feegctid");
		String feegctname = req.getParameter("feegctname");
		...
		try {
			...
			mapRequest.put("feegctid", feegctid);
			mapRequest.put("feegctname", feegctname);
			requestEnvelop.setBody(mapRequest);
			...
			if (returnValue.isSucessFlag()) {
				...
				actionForward
						.setPath("/business/FeeAction.do?method=toAmortize");
	...
	public ActionForward doubleratio(ActionMapping mapping,
		...
		try {
			...
			Date sdate = DateUtil.converToDate(ff.getFeeyear() + "-"
					+ ff.getStartMonth() + "-26");
			Date edate = DateUtil.converToDate(ff.getEndYear() + "-"
					+ ff.getEndMonth() + "-25");
			conn = DBUtil.getConnection();
			fee.setFeestart(sdate);
			fee.setFeeend(edate);
			/*
			 * if(!fee.getFeegctid().equals("")&&fee.getFeegctid()!=null){ proc
			 * = conn.prepareCall("{ call DOUBLERATIO2(?, ?, ?, ?) }");
			 * proc.setString(4, fee.getFeegctid()); }else{ proc =
			 * conn.prepareCall("{ call DOUBLERATIO3(?, ?, ?) }"); }
			 * proc.setDate(2, (java.sql.Date) sdate); proc.setDate(3,
			 * (java.sql.Date) edate); proc.registerOutParameter(1,
			 * Types.DOUBLE); proc.execute(); double retValue =
			 * 100*proc.getDouble(1); DecimalFormat df = new
			 * DecimalFormat("######0.00"); String result = df.format(retValue);
			 * String msg ="从"+sdate+"到"+edate+"的双耳率为 "+result+"%";
			 * super.saveSuccessfulMsg(req, msg);
			 */
			// 获得某区域的各门店的销售台数，销售人数，销售金额，存入数据库
			// 获取服务接口
			BusinessFacade facade = (BusinessFacade) getService("BusinessFacade");
			RequestEnvelop requestEnvelop = new RequestEnvelop();
			EventResponse returnValue = new EventResponse();
			...
			requestEnvelop.setBody(mapRequest);
			// 调用对应的Facade业务处理方法
			ResponseEnvelop resEnv = facade.earCompute(requestEnvelop);
			// 处理返回结果
			returnValue = processRevt(resEnv);
			if (returnValue.isSucessFlag()) {
			...
			File reportFile = new File(req.getSession().getServletContext()
					.getRealPath("\\WEB-INF\\report\\report_ear.jasper"));

			// 传递报表中用到的参数值
			Map<String, Object> parameters = new HashMap<String, Object>();
			// "Name"是报表中定义过的一个参数名称,其类型为String 型
			parameters.put("GCTAREA", ff.getGctarea());
			// 连接到数据库
			// conn = DBUtil.getConnection();
			ServletOutputStream ouputStream = res.getOutputStream();
			String reportclass = "doubleratio";
			res.setContentType("application/vnd.ms-excel");
			res.setHeader("content-disposition", "attachment;filename="
					+ reportclass + ".xls");
	...
	// 单耳单价
	public ActionForward price(ActionMapping mapping, ActionForm actionForm,
			HttpServletRequest req, HttpServletResponse res) throws Exception {
		Fee fee = new Fee();
		FeeForm ff = (FeeForm) actionForm;
		Connection conn = null;
		try {
			...
			Date edate = DateUtil.converToDate(ff.getEndYear() + "-"
					+ ff.getEndMonth() + "-1");
			conn = DBUtil.getConnection();
			if (!fee.getFeegctid().equals("") && fee.getFeegctid() != null) {
				proc = conn.prepareCall("{ call PRICE_CLIENT(?, ?, ? , ?) }");
				proc.setString(4, fee.getFeegctid());
			} else {

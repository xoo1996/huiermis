...
//			}
			...
			charge.setNcdnum(dtoList.get(0).getNcdnum());
...
//			HashMap<String, Object> retmap = new HashMap<String, Object>();
	 ...
	 * 优惠收费信息保存
	 */
	public ResponseEnvelop save4(RequestEnvelop request)
	{
		ResponseEnvelop response = new ResponseEnvelop();
		HashMap<String, Object> retmap = new HashMap<String, Object>();
		Connection con = null;
...
//			HashMap<String, Object> retmap = new HashMap<String, Object>();
			...
			response.setHead(ExceptionSupport(className, "save4",ManageErrorCode.SQLERROR, ex.getMessage(), request.getHead()));
		} finally {
			DBUtil.rollback(con);
			DBUtil.closeConnection(con);
		}
...
/*				if(ChargeAction.checkAcc(id)){
					dto.setFdtprc(dto.getBalance());
					dto.setDiscount(1.0);
				}
                //修改扣率
...
//	}
	...
	public ResponseEnvelop exaNomRec(RequestEnvelop request)
	{
		...
		try {
			...
			for(Charge dto:chgList)
			{
				String sql="select l.ncdsta,l.ncdmon,l.ncdqnt,l.ncddis from tblnorchgdetail l where l.ncdid='" +dto.getNcdid()+"' and l.ncdpid='"+dto.getNcdpid()+"'";	
				List result = (Vector)DBUtil.querySQL(con, sql);
				String ncdsta = (String) ((HashMap) result.get(0)).get("1");
				
				String sql0="select g.chggcltid,g.chgcltid from tblnorchg g where g.chgid='" +dto.getNcdid()+"'";	
				...
				List result4 = (Vector)DBUtil.querySQL(con, "select p.pdtnm,p.pdtcls,p.pdtprc from tblproduct p where p.pdtid='" +
						dto.getNcdpid()+ "'");
				String cls="",pdtprc="";
						if(result4.size() > 0){
							String pdtnm = ((HashMap)result4.get(0)).get("1").toString();
							cls = ((HashMap)result4.get(0)).get("2").toString();
							pdtprc = ((HashMap)result4.get(0)).get("3").toString();
							dto.setStoproname(pdtnm);
						}
				...
				dto.setFileKey("store_insert");
				store(con, dto, null, 0);
				if(ChargeAction.checkAcc(chggcltid)&&("BTE".equals(cls))){
					try {
						String url = ChargeAction.huier;
						String method = "norRec";
						String ncdmon = ((HashMap) result.get(0)).get("2").toString();
						String ncdqnt = ((HashMap) result.get(0)).get("3").toString();
						String ncddis = ((HashMap) result.get(0)).get("4").toString();
						//Double price=Double.parseDouble(ncdmon)/Integer.parseInt(ncdqnt);
						double price=Double.parseDouble(pdtprc)*Double.parseDouble(ncddis);
						String chgcltid = ((HashMap) result0.get(0)).get("2").toString();
						Object[] opAddEntryArgs=new Object[] {dto.getNcdid(),dto.getNcdrnt(),""+pdtprc,dto.getNcdpid(),chgcltid};
						String del=(String)ChargeAction.sendService(opAddEntryArgs, url, method);
						//if(del.equals("0")){
						//	throw new AppException("删除待生成积分失败");
						//}
					}catch(Exception e){
		...
		try {
			...
			if("e".equals(tp))
			{
				for(Order dto:odList)
				{
					...
					modify(con, dto, null, 0);
					dto.setFdtexadt(DateUtil.getSystemCurrentTime());
					...
					//积分商城项目删除待生成积分，根据订单,东新惠耳有效
					/*if(ChargeAction.checkAcc(folctid)){
						try {
							String url = ChargeAction.huier;
							String method = "delwaitscore";
							Object[] opAddEntryArgs=new Object[] {dto.getFolno(),dto.getFolctid()};
							String del=(String)ChargeAction.sendService(opAddEntryArgs, url, method);
							//if(del.equals("0")){
							//	throw new AppException("删除待生成积分失败");
							//}
						}catch(Exception e){
							throw new AppException("连接积分商城系统失败");
						}
					}*/
					//liangping-20171204注释掉，因为定制机审核通过报错
				}
			}
			else if("r".equals(tp))
			{
	...
	public ResponseEnvelop charge3(RequestEnvelop request)
	{
		...
		try {
			...
			if (dto.getRepfolid() == null || dto.getRepfolid().equals("")) {	//dto. 后面要修改
				...
				modify(con, dto, null, 0);
				dto.setFdtdprc(dto.getRepamt());//12-27修改为更新fdtprc原先为fdtdprc

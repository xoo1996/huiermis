...
public class ProductImp extends IMPSupport implements ProductFacade {
	...
	public ResponseEnvelop batchSave(RequestEnvelop request){
		...
		try {
	        ...
	        //lp获取需要更新的个数
	        String sql1="select count(*) as count from tblpnlcfg where cfgpnlnm='" + pnlnm +"'";
	        Statement pstmt2;
			pstmt2 = con.createStatement();
			ResultSet rs2 = pstmt2.executeQuery(sql1);
			int oldLength = 0;
			while(rs2.next()){
				oldLength = Integer.valueOf(rs2.getString("count"));
			}
...
//	        	}
	      ...
	      //lp插入新的零件
	        for(int k = oldLength; k< idList.length; k++){
	        	oldpid=idList[k];
	        	dtoList.get(j).setCfgacyid(oldpid);
            	dtoList.get(j).setCfgpnlnm(pnlnm);
            	dtoList.get(j).setFileKey("pdt05_007");
            	Configuration configuration = dtoList.get(j);
...
//			Store store = new Store();
			...
			{
			   sql+="and p.pdtcls not in('CIC','ITC','HS','ITE') and p.pdtid not in (select aaa102 from aa10 where aaa100='TMEPID') and p.pdtid<>'000001' and p.pdtispnl is null";
			}
			...
			else if(null!=proType && proType.equals("cus2"))
			{
		       sql+="and p.pdtcls in('CIC','ITC','HS','ITE') and (p.pdtnt not like '%停用%' or p.pdtnt is null) ";
			}
...
//			String proType=(String)map.get("proType");
			...
			String sql="select p.pdtid,p.pdtnm,p.pdtmod from tblproduct p where p.pdtispnl='是' or p.pdtcls = 'OTH' ";
			pstm = con.prepareStatement(sql);
            res = pstm.executeQuery();
            while(res.next())
            {

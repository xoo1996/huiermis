...
public class UserImp extends IMPSupport implements UserFacade {
	...
	public ResponseEnvelop modifySc05(RequestEnvelop request) {
		...
		try {
			...
			int size=0;
			if(groupList==null){
				throw new AppException("请选操作员所在科室");
			}
			con = DBUtil.getConnection();
			DBUtil.beginTrans(con);
			...
			if ("add".equals(function)) {
				...
				if (!groupList[0].equals("") && groupList[0] != null)
					sc05.setBsc008(groupList[0]);
				if (!groupList[1].equals("") && groupList[1] != null)
					sc05.setBsc001(groupList[1]);
				...
				if (!groupList[0].equals("") && groupList[0] != null) {
					if (!sc05.getBsc008().equals(groupList[0])) {
						sc13.setBsc037(sc05.getBsc008());
						flag = "doLog";
					}
					sc05.setBsc008(groupList[0]);
				}
				if (!groupList[1].equals("") && groupList[1] != null) {
					if (!sc05.getBsc001().equals(groupList[1])) {
						sc13.setBsc039(sc05.getBsc001());
						flag = "doLog";
					}
					sc05.setBsc001(groupList[1]);
				}
				if (flag != null && flag.equals("doLog")) {
					...
					sc13.setBsc042(groupList[2]);
					sc13.setBsc043(sc05.getBsc001());
					sc13.setBsc044(groupList[3]);

...
/*		String flagDB = (String) httpservletrequest.getAttribute("flagDB");
		System.out.println("------>...___"+flagDB);*/
		...
		try {
			...
			//用以标记连接新数据库，原理是连接新数据库from的表必定在老数据库中没有，所以如果需要添加，请继续添加index2="表名"，index3以此类推
			QueryInfo queryinfo=(QueryInfo) pagequeryinfo;
			String s =getHqlByKey(httpservletrequest, queryinfo.getHqlKey());
			int index = s.indexOf("MEM_BAT");
			
			if (i < 0 || flag) {
				if(index!=-1)
					i = initRowCount2(httpservletrequest, actionform, pagequeryinfo);
				else
			...
			if(index!=-1)
			 collection = fetchPageData2(httpservletrequest,
					actionform, pagequeryinfo);
			else
				collection = fetchPageData(httpservletrequest,
						actionform, pagequeryinfo);
	 ...
	 * @throws Exception
	 */
	public ActionForward init2(HttpServletRequest httpservletrequest, String forward,
			String hql) throws Exception {
		return a(httpservletrequest, forward, hql, false, GlobalNames.PAGE_SIZE2);
	}
	...
	protected int initRowCount2(HttpServletRequest httpservletrequest,
			ActionForm actionform, PageQueryInfo pagequeryinfo)
			throws Exception {
		RequestEnvelop requestEnvelop = null;
		EventResponse returnValue = null;
		...
		requestEnvelop.setBody(mapRequest);
		// 调用对应的Facade业务处理方法
		ResponseEnvelop resEnv = facade.getCount2(requestEnvelop);
		// 处理返回结果
		returnValue = processRevt(resEnv);
		int i = 0;
		if (returnValue.isSucessFlag()) {
	...
	protected Collection fetchPageData2(HttpServletRequest httpservletrequest,
			ActionForm actionform, PageQueryInfo pagequeryinfo)
			throws Exception {
        Connection con=null;
		...
		if ("expExcelAll".equals(s1) || queryinfo.isRetriveAllDataFlag()) {
			...
			requestEnvelop.setBody(mapRequest);
			// 调用对应的Facade业务处理方法
			ResponseEnvelop resEnv = facade.findBySQL2(requestEnvelop);
			// 处理返回结果
			returnValue = processRevt(resEnv);
			if (returnValue.isSucessFlag()) {
				collection = (ArrayList) ((HashMap) resEnv.getBody())

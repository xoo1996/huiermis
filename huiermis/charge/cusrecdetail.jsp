...
<lemis:body>
	...
	<table class="tableinput">
		...
		<html:form action="/ChargeAction.do?method=saveCustomizedCharge"
			...
			<lemis:codelisteditor type="bm" isSelect="true" label="电池型号"
					redisplay="false" required="true" />
			<lemis:texteditor property="rebatnum" label="退回实物电池数（赠送）" required="true"
					disable="false" maxlength="5" onkeyup="this.value=this.value.replace(/[^\d]/ig,'')"/>		
			<lemis:texteditor property="num"  label="已领电池数" disable="true" 
						required="true" value="<%=num %>"/>	
			</tr>
			<tr>
			<lemis:texteditor property="folnt" label="备注" required="true"
					disable="false" colspan="3" maxlength="80" />
			<lemis:codelisteditor type="foldreason" isSelect="true" label="退机原因"
					redisplay="false" required="true" />

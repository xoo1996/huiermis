...
<%
	...
	batchInput.add(new Editor("text", "pdtxh", "助听器型号", "true"));
	batchInput.add(new Editor("text", "pdtyj", "商品单价", "true"));
	batchInput.add(new Editor("-nnnnn", "fdtqnt", "售出数量", "true"));
	batchInput.add(new Editor("text", "fdtrprc", "实际收费", "true"));
	batchInput.add(new Editor("text", "pdtyouhui", "优惠额", "true"));
	batchInput.add(new Editor("text", "jsid", "机身编码", "true"));
	...
	function save() {
		...
		if (confirm("确实要收费吗？")) {
			...
			window.location.href = "/" + lemis.WEB_APP_NAME
					+ "/charge/ChargeAction.do?method=saveYouhuiCharge&" +
...
<script>
	...
	function compute1(e) {
		...
		var value = $("#fdtqnt" + suffix).val() * $("#pdtyj" + suffix).val()*$("#fdtdisc" + suffix).val();
	...
	function compute2(e) {
		var id = $(e.target).attr("id");
		var suffix = id.substr(7);
		var value = $("#fdtrprc" + suffix).val()/$("#fdtqnt" + suffix).val()/$("#pdtyj" + suffix).val();
	...
	$(document).ready(function(e) {
		$("input[name=pdtid]").val("000001");
		$("input[name=pdtid]").attr('readonly','readonly');
		...
		$.ajax({
			...
			/*  data:"proType=nom", */
			 ...
			 success:function(data){
						$("input[name=pdtxh]").autocomplete(data,{
							max:10,
							matchContains:true,
							formatItem:function(data,i,max){
						...
						$("input[name=pdtxh]").result(function(event, data, formatted) {
							if (data){
								var pid = data.proid;
								...
								var id = $(this).parent().find("input").attr("id");
								var suffix = id.substr(5);
								$("#pdtxh" + suffix).val(pnm);
								$("#pdtyj" + suffix).val(prc);//原价
								//$("#pdtnm" + suffix).val(pnm);
...
<lemis:base />
	...
	<lemis:table topic="商品明细录入"
		...
		mode="checkbox" batchInputMeta="batchInput" orderResult="false"
		 batchInputType="insert" pilot="false" pageSize="2"/>

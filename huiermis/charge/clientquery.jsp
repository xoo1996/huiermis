...
<%
	...
	buttons.put("检查项目收费","jiancha(document.all.tableform)");
	buttons.put("收费","shoufei(document.all.tableform)");
...
<script language="javascript">
	function jiancha(obj) {
		...
		obj.action = '<html:rewrite page="/ChargeAction.do?method=saveChgid&tp=check&"/>' + getAlldata(obj);
		obj.submit();
	};
	function shoufei(obj) {
		...
		obj.action = '<html:rewrite page="/ChargeAction.do?method=saveChgid&tp=normal&"/>' + getAlldata(obj);
		obj.submit();
	};
	function news(obj) {
...
<lemis:body>
	...
	<lemis:query action="/ChargeAction.do?method=clientQuery&tp=normal" editorMeta="editor" topic="查询条件" />	

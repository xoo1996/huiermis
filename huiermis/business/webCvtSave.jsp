...
//	batchInput.add(new Editor("text", "pdtnm", "机型名称", "true"));
	...
	//batchInput.add(new Editor("text", "webfrom", "转至惠耳点", "true"));
...
<script language="javascript">
	$(document).ready(function(){
		 var shops = "<%=session.getServletContext().getAttribute("shopList")%>".replace("{","").replace("}","").split(", ");
			...
			if(shops.length==1)
			{
				$("input[name=webfrom]").val(shops[0].substring(shops[0].indexOf("=")+1,shops[0].length));
				$("input[name=webfrom]").attr("readonly","true");
			}
			
			$("input[name=webfrom]").autocomplete(shops,{
				max:10,
				matchContains:true,
			...
			$("input[name=webfrom]").result(function(event,data,formatted){
				if(data){
					var gid = data[0].substring(0,data[0].indexOf("="));
					var gnm=data[0].substring(data[0].indexOf("=")+1,data[0].length);
					var id = $(this).parent().find("input").attr("id");
					var suffix = id.substr(7);
					$("#webfrom" + suffix).val(gnm);
				}
			});
	});
	function chk(e) {
	...
	function commit(obj) {
		...
		obj.action = '<html:rewrite href="/huiermis/business/BusinessAction.do?method=saveWebSell&tp=c&"/>'+getAlldata(document.all.tableform);
...
<lemis:base />
    ...
    <lemis:title title="总部转诊网络统计" />
	<lemis:table topic="总部转诊网络统计"

...
<%
	...
	editors.add(new Editor("money","prices","耳背机价格不低于"));
	//editors.add(new Editor("text","gcttype","店属类型"));
	editors.add(new Editor("money","pricesbox","定制机价格不低于"));
	editors.add(new Editor("text","gctarea","所属区域"));
	editors.add(new Editor("date","stodate","入库日期"));
	editors.add(new Editor("text","gcttype","团体客户类型"));
	
	Map<String,String> buttons=new LinkedHashMap<String,String>();
	buttons.put("计算", "calculate(document.all.tableform)");
...
<script language="javascript">
	...
	function calculate(obj){
		if(confirm("是否重新计算超期数据？")){
			obj.action='<html:rewrite page="/BusinessAction.do?method=calcuExp"/>';
			obj.submit();
		}	
	};
</script>

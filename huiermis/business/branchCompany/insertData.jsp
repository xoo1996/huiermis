...
<script language="javascript">
		...
		if(gctid != '' && iyear != '' && imonth != ''){
			$.getJSON("/huiermis/business/BranchFinancialAction.do?method=getAccuyearinvoiceamount",
					createInvoiceamountString(), function(data) {
						var accuyearinvoiceamount=data[0].accuyearinvoiceamount;
						accuyearinvoiceamount=Number(accuyearinvoiceamount)+Number(invoiceamount);
	...
	function lastMonth() {
		...
		var imonth = $("input[name=imonth]").val();
		if(gctid != '' && iyear != '' && imonth != ''){
			$.getJSON("/huiermis/business/BranchFinancialAction.do?method=getLastMonth",
					createQueryString(), function(data) {
						$('input[name=premoneyfunds]').val(data[0].premoneyfunds);
						$('input[name=preaccountpayable]').val(data[0].preaccountpayable);
						$('input[name=preyearundisprofits]').val(data[0].preyearundisprofits);
						$('input[name=preaccuprofit]').val(data[0].preaccuprofit);
						$('input[name=preaccuinvoiceamount]').val(data[0].preaccuinvoiceamount);
						$('input[name=preaccuactualsales]').val(data[0].preaccuactualsales);
						$('input[name=preaccucostofsales]').val(data[0].accucostofsales);
						$('input[name=paidincapital]').val(data[0].paidincapital);
						$('input[name=elsereceivables]').val(data[0].elsereceivables);
						$('input[name=inventory]').val(data[0].inventory);
						$('input[name=elseaccountpayable]').val(data[0].elseaccountpayable);
...
<lemis:body>
	<lemis:base />
	<lemis:errors />
	<lemis:title title="录入分公司数据" />
	<lemis:tabletitle title="录入分公司数据" />
	<table class="tableinput">
		<lemis:editorlayout />
		<html:form action="/BranchFinancialAction.do?method=insertReport"
			method="POST">
			<tr>
				<lemis:texteditor property="gctid" label="客户代码" disable="false"
					required="true" />
				<lemis:texteditor property="gctname" label="客户名称" disable="false"
					required="true" />
				<lemis:texteditor property="iyear" label="年" disable="false"
					required="true" maxlength="4" />
			</tr>
			<tr>
				<lemis:texteditor property="imonth" label="月" disable="false"
				onclick="lastMonth()" 	required="true" maxlength="2" />
			</tr>

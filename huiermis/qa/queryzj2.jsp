...
<%
	...
	buttons.put("质检步骤1","test2before(document.all.tableform)");
	buttons.put("质检步骤2","test2(document.all.tableform)");
	...
	hidden.put("qatest11","高频平均值");
	hidden.put("qachk","检验结果");
	hidden.put("qapid","商品代码");
	hidden.put("pdttype","商品品牌类型");
	hidden.put("qadev","检测机器");
	hidden.put("qafxman","放行人");
...
<script language="javascript">
	...
	function test2before(obj) {
		var t = editObj("chk");
		if (!t) {
			return t;
		}
		obj.action = '<html:rewrite page="/QAAction.do?method=test2before&"/>' + getAlldata(obj);
		obj.submit();
	}
	function test2(obj) {
		var t = editObj("chk");
		...
		obj.action = '<html:rewrite page="/QAAction.do?method=test2&"/>' + getAlldata(obj);
		obj.submit();
	}
</script>
...
<lemis:body>
	...
	<lemis:query action="/QAAction.do?method=query&order=test2"

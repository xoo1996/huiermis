...
<%
    ...
    buttons.put("RIC产品申请输入","news(document.forms[0])");
							...
							$("input[name=fdtpid]").result(function(event, data, formatted) {
								if (data){
									...
									var id = $(this).parent().find("input").attr("id");
										...
										$("#pdtnm" + suffix).val(pnm);
										$("#fdtdprc" + suffix).val(prc);
										if(pid=="060219"||pid=="060220"||pid=="060221"){
											alert("RIC产品输入请右下角点击‘RIC产品申请输入’按钮");
											$("#fdtpid" + suffix).val("");
											$("#pdtnm" + suffix).val("");
											$("#fdtdprc" + suffix).val("");
...
/* 	.bind("blur",function(e){
	...
	$("input[name=fdtqnt]").bind("blur",function(e){
	 ...
	 function news(obj) {
		obj.action = '<html:rewrite href="/huiermis/order/OrderAction.do?method=inDetail&type=RIC&"/>';
		obj.submit();
	};
	function commit(obj) {
		if (!checkValue(obj)) {

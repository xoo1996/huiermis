...
<%
	...
	header.add(new Formatter("tmksid", "机身编码"));
	...
	/* header.add(new Formatter("foldctime", "订货时间")); */
	...
	buttons.put("打印3D订单","print3DOrder(document.all.tableform)"); 
	buttons.put("打印条形码","printBarcode(document.all.tableform)");
	buttons.put("审核前打印","printBeforeVerify(document.all.tableform)");
...
<script language="javascript">
	...
	function printBeforeVerify(obj) {	
 			window.location.href = "/"
			+ lemis.WEB_APP_NAME
			+ "/order/OrderAction.do?method=printNorBeforeVerify&folno="
			+ $("input[name=folno]").val() + "&gctnm="
			+ $("input[name=gctnm]").val() + "&foltype="	
			+ $("select[name=foltype]").val() + "&fdtcltnm="
			+ $("input[name=fdtcltnm]").val() + "&start="
			+ $("#start").val() + "&end="
			+ $("#end").val();
	...
	function print3DOrder(obj){
		var t = editObj("chk");
			if (!t) {
				return t;
			}
			else {
				obj.action = '<html:rewrite page="/OrderAction.do?method=print3DOrderReport&tp=e&"/>'+getAlldata(obj);		
				obj.submit();
			}
	};

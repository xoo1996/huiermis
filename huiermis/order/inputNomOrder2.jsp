...
<%
	...
	header.add(new Formatter("fdtcolor","颜色"));
	header.add(new Formatter("fdtlaba","喇叭"));
	header.add(new Formatter("fdtpipe","管子"));
	header.add(new Formatter("fdtdire","方向"));
	...
	batchInput.add(new Editor("text","fdtcolor","颜色","true"));
	batchInput.add(new Editor("text","fdtlaba","喇叭","true"));
	batchInput.add(new Editor("text","fdtpipe","管子","true"));
	batchInput.add(new Editor("text","fdtdire","方向","true"));
	batchInput.add(new Editor("text","fdtinnt","备注","true"));
...
<!-- <script src="/huiermis/js/inputDetail.js"> -->
 ...
 $(document).ready(function(e) {
	 ...
	 $("select[name=fdtlaba]").bind("click",function(e){
			joint(e);
		});
	 $("select[name=fdtpipe]").bind("click",function(e){
			joint(e);
		});
	 $("select[name=fdtdire]").bind("click",function(e){
			joint(e);
		});
	 $("select[name=fdtcolor]").bind("click",function(e){
			joint2(e);
		});
	 function joint(e) {
		 var id = $(e.target).attr("id");
			var suffix = id.substr(7);
			var value =$("#fdtcolor" + suffix).val() +', '+ $("#fdtlaba" + suffix).val()+', '+$("#fdtpipe" + suffix).val()+', '+$("#fdtdire" + suffix).val();
			$("#fdtinnt" + suffix).val(value);

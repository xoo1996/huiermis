...
<!-- <script src="/huiermis/js/inputDetail.js"> -->
 ...
 $(document).ready(function(){
	...
	function commit(obj,isExa,md,lr) {
		...
		var lfile = $("#file_0").val();
		var rfile = $("#file_1").val();
		...
		if (lpid == '' && rpid == '') {
			...
			obj.action = '<html:rewrite href="/huiermis/order/CustomOrderAction.do?method=add&tp=3d&isExa='+isExa+'&"/>'
					+ 'num=1'
					+ '&cid='
					+ cid
					+ '&plr=l&pid='
...
/* 		if($('input[name=pdtsheltp]').attr("dataMeta")=="r")
	...
	/* 	$("pdtsheltp").each(function(i){
		...
		if (lpid == '' && rpid == '') {
			...
			obj.action = '<html:rewrite href="/huiermis/order/CustomOrderAction.do?method=add&tp=3d&type=h&"/>'
					+ 'num=1'
					+ '&cid='
					+ cid
					+ '&plr=l&pid='
...
<lemis:base />
<lemis:body>
	<lemis:title title="3D定制订单录入" />
	<html:form  enctype="multipart/form-data" action="/CustomOrderAction.do" method="POST">
            ...
            <tr>
				...
				<lemis:codelisteditor  type="delayleft" label="延长保修的年限（月）" required="false" /><!-- 该处只能输入整数 -->
			  	<lemis:texteditor property="feeleft"  label="续保费"   disable="false" required="false" maxlength="100"/>
			  	 ...
			  	 <tr><td>左耳附件</td><td><input type="file" id="file_0" name="file[0]" style="width:100%" required="true"/></td></tr>
			</table>   
             ...
             <tr>
				...
				<lemis:codelisteditor  type="delayright" label="延长保修的年限（月）" required="false" /><!-- 该处只能输入整数 -->
			  	<lemis:texteditor property="feeright"  label="续保费"   disable="false" required="false" maxlength="100"/>
			  	 ...
			  	 <tr><td>右耳附件</td><td><input type="file" id="file_1" name="file[1]"  style="width:100%" required="true"/></td></tr>

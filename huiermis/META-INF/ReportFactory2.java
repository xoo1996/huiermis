...
public class ReportFactory2 implements JRDataSource {
	...
	private List<Report> data ;
	 ...
	 public List<Report> setCollection(String gcltid,Integer year,Integer month) throws Exception{
		 ...
		 try {
			...
			String sql2 = "SELECT DISTINCT NVL(SUM(i.ivtpqnt)over(partition BY p.pdtid,i.ivttype order by i.ivttype, p.pdtid),0) AS temp01," +
					...
					" p.pdtid, p.pdtnm, d.disprice, p.pdtprc, p.pdtcls, I.Ivttype, " +
					//商品代码，商品名称，成本扣率？，耳机价格，耳机类别，月结类型
					"Nvl(Sum(S.Changemon)Over(Partition By S.Bsc011,S.Pdtid Order By S.Pdtid),0)*0.04 As score, " +
					"Nvl(Sum(S.Changecoin)Over(Partition By S.Bsc011,S.Pdtid Order By S.Pdtid),0) As coin " +
					"FROM tblinventory i " +
					//月结表
					...
					"WHERE (i.ivtpqnt <>0 OR i.ivtpamnt <>0) And I.Ivttype <>'repair' And I.Ivttype <>'repairEar' AND i.ivtpdtid<>'0805005' AND i.ivtpdtid<>'0805006' AND i.ivtpdtid<>'0805007'  AND i.ivtpdtid<>'0805008' And I.Ivtgcltid ='"+gcltid+"' AND i.ivtyear ='"+year+"' AND i.ivtmonth ='"+month+"' AND (i.ivtnote not in ('加急费','续保费') or i.ivtnote is null) " +
					//积分表
					"UNION ALL SELECT DISTINCT 1 AS temp01, " +
					//加急费部分
					"i.ivtpamnt  AS temp02,  " +
					" p.pdtid, p.pdtnm ||' ' ||i.ivtnote, i.ivtfee AS pdtprc, i.ivtfee, p.pdtcls, i.ivttype, " +
...
*/
			...
			String sql5 ="SELECT NVL(SUM(a.temp1+b.temp2),0) AS ressum " +
					"from (SELECT SUM(i.ivtpqnt*d.disprice) as temp1 FROM tbldiscount d LEFT OUTER JOIN TBLINVENTORY i ON (d.dispdtid =i.ivtpdtid And D.Disgctid =I.Ivtgcltid) WHERE i.IVTTYPE = 'repairEar' AND (i.ivtpqnt <>0 OR i.ivtpamnt <>0) And I.Ivtgcltid ='"+gcltid+"' AND i.ivtyear ='"+year+"' AND i.ivtmonth ='"+month+"') a," +
					"(SELECT SUM(rdisprice) as temp2 FROM tblrepdiscount where rdisgctid='"+gcltid+"' and rdisyear='"+year+"' and rdismonth='"+month+"') b";

...
<%	
	...
	//header.add(new Formatter("band", "商品品牌"));
	...
	buttons.put("导出未开票记录","exportNotBill()");
	buttons.put("生成未开票记录","submitFin(document.all.tableform)");
	...
	hidden.put("retailname", "开票抬头");
	hidden.put("retailtaxno", "开票税号");
...
function submitFin(obj) {
	if (confirm("确认生成未开票开票记录？本操作耗时较长，请在点击确定后，耐心等待，不要切换页面！")) {
		
		var li=$(".buttonGray");
		for(var i=0;i<li.length;i++){
			if(li[i].value=="生成未开票记录") {
				li[i].disabled=true;
			}
		}
		...
		window.location.href = "/"
			+ lemis.WEB_APP_NAME
			+ "/finance/FinanceAction.do?method=saveNotFinance&"
			+ getAlldata(document.all.tableform);
	}
};

	function exportNotBill() {
		var li=$(".buttonGray");
		for(var i=0;i<li.length;i++){
			if(li[i].value=="导出未开票记录") {
				li[i].disabled=true;
			}
		}
		window.location.href = "/"
			+ lemis.WEB_APP_NAME
			+ "/finance/FinanceAction.do?method=exportNotBillExcel";
	};
</script>

<lemis:base />
<lemis:body>
	<lemis:title title="未开票商品提交开票" />
	<lemis:query action="/FinanceAction.do?method=queryToDrawNotBill" editorMeta="editor"
		topic="查询条件" />
